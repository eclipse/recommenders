...
public class NewsJob extends Job {
    ...
    public NewsJob(FeedDescriptor feed, NewsRcpPreferences preferences, NotificationEnvironment environment) {
        super(feed.getId());
        this.feed = feed;
    ...
    public boolean belongsTo(Object job) {
        ...
        if (!job.getClass().isInstance(NewsJob.class)) {
            return false;
        }
        NewsJob rhs = (NewsJob) job;
        if (feed.getId() == null) {
            if (rhs.getFeed().getId() != null) {
                return false;
            }
        } else if (!feed.getId().equals(rhs.getFeed().getId())) {
            return false;
        }
        return true;
    }
