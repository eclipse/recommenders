...
public class FeedDialog extends TitleAreaDialog {
    ...
    private void updateDialog() {
        ...
        if (Strings.isNullOrEmpty(nameValue.getText())) {
            ...
            super.getButton(OK).setEnabled(false);
        } else if (getFeedIds(newsRcpPreferences).contains(urlValue.getText())) {
            setErrorMessage(Messages.FEED_DIALOG_ERROR_DUPLICATE_FEED);
            super.getButton(OK).setEnabled(false);
    ...
    private List<String> getFeedIds(NewsRcpPreferences newsRcpPreferences) {
        List<String> result = Lists.newArrayList();
        for (FeedDescriptor feed : newsRcpPreferences.getFeedDescriptors()) {
            result.add(feed.getId());
        }
        return result;
    }
