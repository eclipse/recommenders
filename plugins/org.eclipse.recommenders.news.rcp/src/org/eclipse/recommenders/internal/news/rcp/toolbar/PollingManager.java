...
import org.eclipse.recommenders.internal.news.rcp.PollFeedsJob;
...
public class PollingManager {
    ...
    @Inject
    public void updateStartupDelay(@Preference(PreferenceConstants.POLLING_DELAY) int pollingDelay) {
        this.pollingDelay = pollingDelay;
    }
    ...
    public synchronized void setEnabled(@Preference(PreferenceConstants.NEWS_ENABLED) boolean enabled) {
        if (enabled) {
            if (job == null) {
                job = new PollFeedsJob(Messages.POLL_FEED_JOB_NAME, true, preferences, pollingService, eventBroker);
