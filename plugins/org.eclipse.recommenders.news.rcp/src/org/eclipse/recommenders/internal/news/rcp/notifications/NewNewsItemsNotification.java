...
public class NewNewsItemsNotification extends AbstractUiNotification {
    ...
    public NewNewsItemsNotification(NewsItem newsItem, FeedDescriptor feed, Object token, ECommandService commandService, EHandlerService handlerService) {
        super(NotificationConstants.EVENT_NEW_NEWS_ITEMS);
    ...
    @Override
    public void open() {
        ParameterizedCommand command = commandService.createCommand(Commands.ID_READ_NEWS_ITEMS, ImmutableMap.<String, Object>of(
                Commands.PARAMETER_READ_NEWS_ITEMS_NEWS_ITEMS, Collections.singleton(newsItem),
                Commands.PARAMETER_READ_NEWS_ITEMS_OPEN_BROWSER, true));
    ...
    public String getDescription() {
        return MessageFormat.format("from {0}", feed.getName());
    }
