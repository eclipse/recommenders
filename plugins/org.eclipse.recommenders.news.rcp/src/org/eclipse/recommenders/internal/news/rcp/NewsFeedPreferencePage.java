...
public class NewsFeedPreferencePage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
    ...
    protected void createFieldEditors() {
        ...
        addField(feedEditor);
        ConfigurationEditor configurationEditor = new ConfigurationEditor(Constants.PREF_FEED_LIST_SORTED,
                Messages.FIELD_LABEL_FEEDS, getFieldEditorParent());
    ...
    private final class ConfigurationEditor extends FieldEditor {
        ...
        protected void doFillIntoGrid(Composite parent, int numColumns) {
            ...
            tableViewer.getTable().addMouseListener(new MouseAdapter() {
                ...
                public void mouseDoubleClick(MouseEvent e) {
                    ...
                    if (!feed.isExtensionPoint()) {
                        editConfiguration(feed);
                    }
                    updateButtonStatus();
                }
            });
        ...
        private void updateButtonStatus() {
            ...
            FeedDescriptor feed = getSelectedConfiguration();
            if (feed == null) {
                editButton.setEnabled(false);
                removeButton.setEnabled(false);
                return;
            }
            editButton.setEnabled(selected && !feed.isExtensionPoint());
            removeButton.setEnabled(selected && !feed.isExtensionPoint());
        }
        ...
        private FeedDescriptor getSelectedConfiguration() {
        ...
        protected void removeConfiguration(FeedDescriptor feed) {
            List<FeedDescriptor> feeds = getTableInput();

            MessageDialogWithToggle confirmDialog = MessageDialogWithToggle.openOkCancelConfirm(
                    NewsFeedPreferencePage.this.getShell(), Messages.CONFIRM_DIALOG_DELETE_FEED_TITLE,
                    format(Messages.CONFIRM_DIALOG_DELETE_FEED_MESSAGE, feed.getName()),
                    Messages.CONFIRM_DIALOG_DELETE_FEED_TOGGLE_MESSAGE, true, null, null);
        ...
        protected void editConfiguration(FeedDescriptor oldFeed) {
            FeedDialog dialog = new FeedDialog(getShell(), oldFeed);
            List<FeedDescriptor> feeds = getTableInput();
