...
public class NewsNotificationPopup extends AbstractNotificationPopup {
    ...
    private static final int DEFAULT_NOTIFICATION_MESSAGES = 3;
    ...
    private void processNotificationData(Composite composite, Map<FeedDescriptor, List<IFeedMessage>> sortedMap) {
        ...
        for (Entry<FeedDescriptor, List<IFeedMessage>> entry : sortedMap.entrySet()) {
            if (feedCounter < DEFAULT_NOTIFICATION_MESSAGES) {
                ...
                processMessages(composite, entry.getValue());
            }
            feedCounter++;
        }
    }

    private void processMessages(Composite composite, List<IFeedMessage> messages) {
        int messageCounter = 0;
        for (final IFeedMessage message : messages) {
            if (messageCounter < DEFAULT_NOTIFICATION_MESSAGE_PER_FEED) {
