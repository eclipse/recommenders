...
public class NewsNotificationPopup extends AbstractNotificationPopup {
    ...
    private final Map<IFeed, IPollingResult> messages;
    private final EventBus eventBus;
    @Inject
    private IFeedEvents feedEvents;

    public NewsNotificationPopup(Display display, Map<IFeed, IPollingResult> messages, EventBus eventBus) {
        super(display);
    ...
    protected void createContentArea(Composite composite) {
        ...
        Map<IFeed, IPollingResult> sortedMap = MessageUtils.sortByDate(messages);
    ...
    private void processNotificationData(Composite composite, Map<IFeed, IPollingResult> sortedMap) {
        int feedCounter = 0;
        int messagesPerFeed = DEFAULT_NOTIFICATION_MESSAGES < sortedMap.size() ? 1
                : DEFAULT_NOTIFICATION_MESSAGES / sortedMap.size();
        for (Entry<IFeed, IPollingResult> entry : sortedMap.entrySet()) {
