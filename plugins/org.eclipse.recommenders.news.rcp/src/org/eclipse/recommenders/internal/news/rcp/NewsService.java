...
public class NewsService implements INewsService {
    ...
    public void jobDone(IPollFeedJob job) {
        ...
        messages.values();
        // those two loops below remove message IDs that are not present in message list from job
        ...
        // Iterator<String> readIdIter = readIds.iterator();
        for (Iterator<String> readIdIter = readIds.iterator(); readIdIter.hasNext();) {
            if (!allMessages.contains(readIdIter)) {
                readIdIter.remove();
            }
        }
        for (Map.Entry<FeedDescriptor, List<IFeedMessage>> entry : messages.entrySet()) {
