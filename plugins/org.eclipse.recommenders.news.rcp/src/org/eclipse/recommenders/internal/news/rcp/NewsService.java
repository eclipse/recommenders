...
public class NewsService implements INewsService {
    ...
    public void handleFeedRead(FeedReadEvent event) {
        List<IFeedMessage> messages = getMessages(Constants.COUNT_PER_FEED).get(event.getFeed());
        Set<String> readIds = Sets.newHashSet();
        for (IFeedMessage message : messages) {
            readIds.add(message.getId());
        }
        this.readIds.addAll(readIds);
        newsFeedProperties.writeReadIds(this.readIds);
    }

    @Subscribe
    @Override
    public void handleAllRead(AllReadEvent event) {
        Map<FeedDescriptor, List<IFeedMessage>> map = getMessages(Constants.COUNT_PER_FEED);
        Set<String> readIds = Sets.newHashSet();
