...
public class ReadNewsItemsHandler {
    ...
    @Inject
    public ReadNewsItemsHandler(IReadItemsStore readItemsStore, ECommandService commandService, EHandlerService handlerService, UISynchronize uiSynchronize) {
    ...
    @CanExecute
    public boolean canExecute(@Named(Commands.COMMAND_PARAM_READ_NEWS_ITEMS_NEWS_ITEMS) final List<NewsItem> newsItems) {
        return !newsItems.isEmpty();
    }

    @Execute
    public void execute(@Named(Commands.COMMAND_PARAM_READ_NEWS_ITEMS_NEWS_ITEMS) final List<NewsItem> newsItems,
