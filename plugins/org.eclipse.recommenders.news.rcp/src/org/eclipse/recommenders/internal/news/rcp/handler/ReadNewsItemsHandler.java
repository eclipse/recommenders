...
public class ReadNewsItemsHandler {
    ...
    @CanExecute
    public boolean canExecute(@Named(Commands.PARAMETER_READ_NEWS_ITEMS_NEWS_ITEMS) final List<NewsItem> newsItems) {
        return !newsItems.isEmpty();
    }

    @Execute
    public void execute(@Named(Commands.PARAMETER_READ_NEWS_ITEMS_NEWS_ITEMS) final Collection<NewsItem> newsItems,
            @Optional @Nullable @Named(Commands.PARAMETER_READ_NEWS_ITEMS_OPEN_BROWSER) final Boolean openBrowser) {
        new Job(Messages.JOB_NAME_READ_NEWS_ITEMS) {
            ...
            protected IStatus run(IProgressMonitor monitor) {
                ...
                eventBroker.post(Topics.NEWS_ITEMS_READ, newsItems);
            ...
            private void openBrowser(final NewsItem newsItem) {
                uiSynchronize.asyncExec(new Runnable() {
                    ...
                    @Override
                    public void run() {
                        ParameterizedCommand command = commandService.createCommand(Commands.ID_OPEN_BROWSER,
                                ImmutableMap.<String, Object>of(Commands.PARAMETER_OPEN_BROWSER_URL,
