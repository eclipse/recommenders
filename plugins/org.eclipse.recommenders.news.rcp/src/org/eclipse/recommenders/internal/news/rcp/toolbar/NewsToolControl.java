...
import org.eclipse.recommenders.internal.news.rcp.Topics;
...
public class NewsToolControl {
        ...
        contextMenu.addMenuListener(new IMenuListener() {
            ...
            public void menuAboutToShow(IMenuManager menu) {
                ...
                ExecuteCommandAction pollNowAction = new ExecuteCommandAction(Messages.LABEL_POLL_FEEDS,
                        CommonImages.REFRESH, Commands.ID_POLL_NEWS_FEEDS, Collections.<String, Object>emptyMap());
                menu.add(pollNowAction);

                ExecuteCommandAction markAsReadAction = new ExecuteCommandAction(Messages.LABEL_MARK_AS_READ, null,
                        Commands.ID_READ_NEWS_ITEMS, ImmutableMap.<String, Object>of(
                                Commands.PARAMETER_READ_NEWS_ITEMS_NEWS_ITEMS, getAllItems(feedContents.values())));
                menu.add(markAsReadAction);

                menu.add(new Separator());

                ExecuteCommandAction preferencesAction = new ExecuteCommandAction(Messages.LABEL_PREFERENCES, null,
                        Commands.ID_PREFERENCES, ImmutableMap.<String, Object>of(
                                Commands.PARAMETER_PREFERENCES_PREFERENCE_PAGE_ID, Constants.PREF_PAGE_ID));
    ...
    private MenuManager createFeedMenu(FeedDescriptor feed, PollingResult result) {
        ...
        for (int i = 0; i < MessageAge.values().length; i++) {
            ...
            for (NewsItem item : ageGroup) {
                ...
                ExecuteCommandAction readNewsItemAction = new ExecuteCommandAction(itemLabel, null,
                        Commands.ID_READ_NEWS_ITEMS,
                        ImmutableMap.<String, Object>of(Commands.PARAMETER_READ_NEWS_ITEMS_NEWS_ITEMS,
                                Collections.singleton(item), Commands.PARAMETER_READ_NEWS_ITEMS_OPEN_BROWSER, true));
        ...
        ExecuteCommandAction markAsReadAction = new ExecuteCommandAction(Messages.LABEL_MARK_AS_READ, null,
                Commands.ID_READ_NEWS_ITEMS,
                ImmutableMap.<String, Object>of(Commands.PARAMETER_READ_NEWS_ITEMS_NEWS_ITEMS, items));
    ...
    @Inject
    @Optional
    public void handlePollingResults(@UIEventTopic(Topics.POLLING_RESULTS) Collection<PollingResult> ignored) {
        refreshUnreadItemsStatus(openContextMenuAction);
    }

    @Inject
    @Optional
    public void handleNewsItemsRead(@UIEventTopic(Topics.NEWS_ITEMS_READ) Collection<NewsItem> ignored) {
        refreshUnreadItemsStatus(openContextMenuAction);
    }

    private void refreshUnreadItemsStatus(Action action) {
        boolean hasUnreadItems = hasUnreadItems(
    ...
    private boolean hasUnreadItems(Collection<NewsItem> items) {
        for (NewsItem item : items) {
            if (!readItemsStore.isRead(item)) {
                return true;
            }
