...
public class NewsToolbarContribution extends WorkbenchWindowControlContribution {
    ...
    private final int COUNT_PER_FEED = 3;
        ...
        PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
            ...
            public void run() {
                ...
                if (preferences.isNotificationEnabled()) {
                    new NewsNotificationPopup(getLatestMessages(service.getMessages(COUNT_PER_FEED)), eventBus).open();
                }
            }
        });
    }
    ...
    private Map<FeedDescriptor, List<IFeedMessage>> getLatestMessages(
        ...
        for (Entry<FeedDescriptor, List<IFeedMessage>> entry : messages.entrySet()) {
            ...
            if (entry.getValue().get(0).getDate()
                    .after(properties.getDates(Constants.FILENAME_FEED_DATES).get(entry.getKey().getId()))) {
                iFeedMessages.addAll(entry.getValue());
            } else if (entry.getValue().get(0).getDate()
                    .equals(properties.getDates(Constants.FILENAME_FEED_DATES).get(entry.getKey().getId()))) {
                for (int i = 0; i < entry.getValue().size() - 1; i++) {
                    iFeedMessages.add(entry.getValue().get(i));
                }
            }
            result.put(entry.getKey(), iFeedMessages);
        }
        return result;
    }
