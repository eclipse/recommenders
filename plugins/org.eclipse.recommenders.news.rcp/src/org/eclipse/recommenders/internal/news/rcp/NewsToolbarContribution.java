    ...
    public void handle(NewFeedItemsEvent event) {
        PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
            ...
            public void run() {
                ...
                Map<FeedDescriptor, List<IFeedMessage>> latestMessages = removeEmptyFeeds(
                        getLatestMessages(service.getMessages(COUNT_PER_FEED)));
                if (service.displayNotification(latestMessages)) {
                    new NewsNotificationPopup(latestMessages, eventBus, service, Shells.getDisplay()).open();
                }
            }
        });
    }
