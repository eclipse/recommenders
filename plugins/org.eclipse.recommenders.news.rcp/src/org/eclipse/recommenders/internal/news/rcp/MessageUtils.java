...
public class MessageUtils {
    ...
    public static Map<FeedDescriptor, PollingResult> toMapValuePollingResult(
            Map<FeedDescriptor, List<IFeedMessage>> map) {
        Map<FeedDescriptor, PollingResult> result = Maps.newHashMap();
        for (Map.Entry<FeedDescriptor, List<IFeedMessage>> entry : map.entrySet()) {
            result.put(entry.getKey(), new PollingResult(entry.getValue()));
        }
        return result;
    }
