...
public class NewsMenuListener implements IMenuListener {
    ...
    private void addMessages(MenuManager menu, List<IFeedMessage> messages) {
        for (final IFeedMessage message : messages) {
            ...
            if (!message.isRead()) {
                action.setText(MessageFormat.format(Messages.UNREAD_MESSAGE_PREFIX, message.getTitle()));
            } else {
                action.setText(MessageFormat.format(Messages.READ_MESSAGE_PREFIX, message.getTitle()));
            }
            menu.add(action);
        }
    }
    ...
    private static String getMenuEntryTitle(String feedName, List<IFeedMessage> messages) {
        if (getUnreadMessagesNumber(messages) > 0) {
            return MessageFormat.format(Messages.UNREAD_MESSAGE_PREFIX,
                    feedName + " (" + getUnreadMessagesNumber(messages) + ")"); //$NON-NLS-1$ //$NON-NLS-2$
        } else {
            return MessageFormat.format(Messages.READ_MESSAGE_PREFIX, feedName);
        }
    }
