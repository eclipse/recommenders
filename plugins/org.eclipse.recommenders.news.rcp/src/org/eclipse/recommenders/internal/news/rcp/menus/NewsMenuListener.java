...
public class NewsMenuListener implements IMenuListener {
    ...
    public void menuAboutToShow(IMenuManager manager) {
        for (Entry<FeedDescriptor, List<IFeedMessage>> entry : messages.entrySet()) {
            ...
            addMarkAsReadAction(entry, menu);
            manager.add(menu);
        }
        manager.add(new Separator());
        manager.add(new MarkAsReadAction(eventBus, null, true));
    }

    private void addMarkAsReadAction(Entry<FeedDescriptor, List<IFeedMessage>> entry, MenuManager menu) {
        Map<FeedDescriptor, List<IFeedMessage>> mapFromEntry = Maps.newHashMap();
        mapFromEntry.put(entry.getKey(), entry.getValue());
        menu.add(new Separator());
        menu.add(new MarkAsReadAction(eventBus, entry.getKey(), false));
    }
