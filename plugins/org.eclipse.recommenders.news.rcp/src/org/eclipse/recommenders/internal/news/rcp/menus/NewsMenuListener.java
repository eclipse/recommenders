...
public class NewsMenuListener implements IMenuListener {
    ...
    public void menuAboutToShow(IMenuManager manager) {
        for (Entry<FeedDescriptor, List<IFeedMessage>> entry : messages.entrySet()) {
            String menuName = processFeedName(entry.getKey().getName(), entry.getValue());
    ...
    private static String processFeedName(String feedName, List<IFeedMessage> messages) {
        boolean read = false;
        ...
        if (!unread && read) {
            ...
            return Messages.UNREAD_MESSAGE_PREFIX.concat(feedName)
                    .concat(" (" + getUnreadMessagesNumber(messages) + ")");
        } else {
            return Messages.PARTIALLY_READ_FEED.concat(feedName).concat(" (" + getUnreadMessagesNumber(messages) + ")");
        }
    }
