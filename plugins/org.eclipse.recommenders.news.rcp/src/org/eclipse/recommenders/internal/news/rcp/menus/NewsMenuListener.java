...
public class NewsMenuListener implements IMenuListener {
    ...
    private void addMessages(MenuManager menu, List<IFeedMessage> messages, final FeedDescriptor feed) {
        for (final IFeedMessage message : messages) {
            ...
            if (message.getTitle().contains("@")) { //$NON-NLS-1$
                title = message.getTitle() + "\t"; //$NON-NLS-1$
            }
            if (!message.isRead()) {
                action.setText(
                        MessageFormat.format(Messages.UNREAD_MESSAGE, title == null ? message.getTitle() : title));
            } else {
                action.setText(MessageFormat.format(Messages.READ_MESSAGE_OR_FEED,
