...
public class NewsMenuListener implements IMenuListener {
    ...
    private static final String UNREAD_MESSAGE_PREFIX = "* ";
    ...
    public void menuAboutToShow(IMenuManager manager) {
        for (Entry<FeedDescriptor, List<IFeedMessage>> entry : messages.entrySet()) {
            ...
            for (final IFeedMessage message : entry.getValue()) {
                ...
                if (!message.isRead()) {
                    action.setText(UNREAD_MESSAGE_PREFIX.concat(action.getText()));
                }
                menu.add(action);
            }
            addMarkAsReadMenuListener(entry, menu);
            manager.add(menu);
        }
    }

    private void addMarkAsReadMenuListener(Entry<FeedDescriptor, List<IFeedMessage>> entry, MenuManager menu) {
        MarkAsReadMenuListener markAsReadMenuListener = new MarkAsReadMenuListener(eventBus);
        Map<FeedDescriptor, List<IFeedMessage>> mapFromEntry = Maps.newHashMap();
        mapFromEntry.put(entry.getKey(), entry.getValue());
        markAsReadMenuListener.setMessages(mapFromEntry);
        menu.addMenuListener(markAsReadMenuListener);
    }
