...
public class NewsMenuListener implements IMenuListener {
    ...
    private void addMessages(MenuManager menu, List<IFeedMessage> messages) {
        for (final IFeedMessage message : messages) {
            ...
            if (!message.isRead()) {
                ...
                action.setText(MessageFormat.format(Messages.READ_MESSAGE, message.getTitle()));
            }
            menu.add(action);
        }
    }
    ...
    private static String getMenuEntryTitle(String feedName, List<IFeedMessage> messages) {
        ...
        if (unreadMessages > 0) {
            return MessageFormat.format(Messages.UNREAD_MESSAGE, feedName, unreadMessages);
        } else {
            return MessageFormat.format(Messages.READ_MESSAGE, feedName);
        }
    }
