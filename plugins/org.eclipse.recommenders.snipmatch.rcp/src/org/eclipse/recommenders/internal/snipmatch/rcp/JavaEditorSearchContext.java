...
public class JavaEditorSearchContext extends SearchContext {
    ...
    private static Set<ProjectCoordinate> getAvailableDependencies(JavaContentAssistInvocationContext invocationContext) {
        ...
        IProjectCoordinateAdvisorService pcAdvisor = InjectionService.getInstance().requestInstance(
                IProjectCoordinateAdvisorService.class);
        ...
        for (DependencyInfo dependencyInfo : availableDependencies) {
            ProjectCoordinate pc = pcAdvisor.suggest(dependencyInfo).orNull();
            if (pc != null) {
                result.add(pc);
            }
        }
