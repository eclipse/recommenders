...
public class CreateSnippetHandler extends AbstractHandler {
    ...
    @VisibleForTesting
    protected Snippet createSnippet(CompilationUnitEditor editor) throws ExecutionException {
        this.editor = editor;
    ...
    private void addImport(ITypeBinding type) {
        ...
        ITypeBinding earasure = type.getErasure();
        String qualifiedName = earasure.getQualifiedName();
    ...
    private void appendVariableBinding(SimpleName name, IVariableBinding vb) {
        ...
        try {
            for (ISnippetRepository r : repos) {
                if (r.isImportSupported()) {
                    final SnippetEditorInput input = new SnippetEditorInput(snippet, r);
                    HandlerUtil.getActiveWorkbenchWindow(event).getActivePage().openEditor(input, EDITOR_ID);
                    return;
                }
            }
            openError(HandlerUtil.getActiveShell(event), "No editable snippet repository found",
                    "Your configuration does not contain any editable code snippet repository. Please create one first.");
