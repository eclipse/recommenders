...
public final class SelectRepositoryDialog {
	...
	public static Optional<ISnippetRepository> openSelectRepositoryDialog(
			final Shell shell, Repositories repos,
	...
	public static Optional<ISnippetRepository> openSelectRepositoryDialog(
			final Shell shell, Repositories repos,
			SnippetRepositoryConfigurations configs,
			SnippetRepositoryConfiguration preSelectedConfiguration) {

		List<SnippetRepositoryConfiguration> filteredConfigurations = filterImportSupportingConfigurations(
				repos, configs);
		if (filteredConfigurations.isEmpty()) {
			return absent();
		}

		ListDialog selectRepositoryDialog = createSelectRepositoryDialog(shell,
				filteredConfigurations, preSelectedConfiguration, repos);

		int status = selectRepositoryDialog.open();
		if (status == Status.OK) {
			SnippetRepositoryConfiguration config = cast(selectRepositoryDialog
					.getResult()[0]);
	...
	private static ListDialog createSelectRepositoryDialog(final Shell shell,
		...
		selectRepositoryDialog
				.setTitle(Messages.SELECT_REPOSITORY_DIALOG_TITLE);
		selectRepositoryDialog
				.setMessage(Messages.SELECT_REPOSITORY_DIALOG_MESSAGE);
		...
		if (preSelectedConfiguration != null
				&& isImportSupported(preSelectedConfiguration, repos)) {
		...
		if (selectedElement != null) {
			selectRepositoryDialog
					.setInitialSelections(new SnippetRepositoryConfiguration[] { selectedElement });
		}
	...
	private static boolean isImportSupported(
			SnippetRepositoryConfiguration configuration, Repositories repos) {
		ISnippetRepository repo = repos.getRepository(configuration.getId())
				.orNull();
		return repo != null && repo.isImportSupported();
	}

	private static List<SnippetRepositoryConfiguration> filterImportSupportingConfigurations(
			Repositories repos, SnippetRepositoryConfigurations configs) {

		List<SnippetRepositoryConfiguration> filteredConfigurations = Lists
				.newArrayList();
