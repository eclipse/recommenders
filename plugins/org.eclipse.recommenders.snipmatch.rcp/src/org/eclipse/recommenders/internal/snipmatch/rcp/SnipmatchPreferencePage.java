    ...
    public void createFieldEditors() {
        snippetsRepoField = new StringButtonFieldEditor(PREF_SNIPPETS_REPO,
            ...
            private EclipseGitSnippetRepositoryConfiguration loadValue(String string) {
                ISnippetRepositoryConfiguration[] configurations = RepositoryConfigurations.convert(string, providers);
                if (configurations.length > 0 && configurations[0] instanceof EclipseGitSnippetRepositoryConfiguration) {
                    EclipseGitSnippetRepositoryConfiguration config = cast(configurations[0]);
                    return config;
                }
                return null;
            }
            ...
            @Override
            public void store() {
                ISnippetRepositoryConfiguration config = new EclipseGitSnippetRepositoryConfiguration(getTextControl()
                        .getText(), true);
                getPreferenceStore().setValue(PREF_SNIPPETS_REPO, RepositoryConfigurations.convert(config, providers));
            }
