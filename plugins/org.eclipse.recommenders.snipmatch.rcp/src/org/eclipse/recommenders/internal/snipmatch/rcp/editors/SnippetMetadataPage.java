...
public class SnippetMetadataPage extends FormPage {
    ...
    private Button btnAddFilename;
    private Button btnRemoveFilename;
    ...
    private IObservableList ppFilenames;
    ...
    protected void createFormContent(IManagedForm managedForm) {
        ...
        contentsPart = new AbstractFormPart() {
            ...
            public void initialize(IManagedForm managedForm) {
                ...
                listViewerFilenames = createListViewer(managedForm, horizontalIndent);
                createDecoration(listViewerFilenames.getList(), Messages.EDITOR_DESCRIPTION_FILENAME_RESTRICTIONS,
                        infoDecorationImage, SWT.TOP | SWT.LEFT);

                filenamesButtonContainer = createButtonContainer(managedForm);
                btnAddFilename = createButton(managedForm, filenamesButtonContainer, Messages.EDITOR_BUTTON_ADD,
                        new SelectionAdapter() {
                    @Override
                    public void widgetSelected(SelectionEvent e) {
                        createFilenameInputDialog(filenamesButtonContainer.getShell()).open();
                    }
                });
                btnAddFilename.setEnabled(snippet.getLocation() == Location.FILE);
                btnRemoveFilename = createButton(managedForm, filenamesButtonContainer, Messages.EDITOR_BUTTON_REMOVE,
                        new SelectionAdapter() {
                    @Override
                    public void widgetSelected(SelectionEvent e) {
                        Optional<String> o = Selections.getFirstSelected(listViewerFilenames);
                        if (o.isPresent()) {
                            ppFilenames.remove(o.get());
                        }
                    }
                });
    ...
    private InputDialog createFilenameInputDialog(Shell shell) {
        ...
        return new InputDialog(shell, Messages.DIALOG_TITLE_ENTER_NEW_FILENAME_RESTRICTION,
            ...
            protected void okPressed() {
                ppFilenames.add(getValue().toLowerCase());
                super.okPressed();
            }
        };
    }
    ...
    private DataBindingContext createDataBindingContext() {
        ...
        // Filenames
        ppFilenames = BeanProperties.list(Snippet.class, "filenameRestrictions", String.class) //$NON-NLS-1$
                .observe(snippet);
        ppFilenames.addChangeListener(new ContentsPartDirtyListener());
        ViewerSupport.bind(listViewerFilenames, ppFilenames, new FilenameRestrictionLabelProperty());
        ...
        final IObservableValue vpFilenamesSelection = singleSelection().observe(listViewerFilenames);
        IObservableValue wpBtnRemoveFilenamesEnable = enabled().observe(btnRemoveFilename);

        IObservableValue vpLocationSelection = ViewersObservables.observeSingleSelection(comboLocation);
        IObservableValue wpListViewerFilenamesEnabled = enabled().observe(listViewerFilenames.getControl());
        final IObservableValue wpBtnAddFilenamesEnabled = enabled().observe(btnAddFilename);
