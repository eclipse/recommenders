...
public class SnippetsView extends ViewPart implements IRcpService {
    ...
    private void shareSnippets() {
        ...
        Collection<UUID> selectedUuids = Collections2.transform(selectedSnippets, new Function<KnownSnippet, UUID>() {
            @Override
            public UUID apply(KnownSnippet input) {
                return input.snippet.getUuid();
            }
        });
        repository.get().share(selectedUuids);
    }
    ...
    private boolean selectionIsShareable() {
        ...
        selectionConsistsOnlyElementsOf(KnownSnippet.class);
        int configId = -1;
        for (Object element : selection) {
            KnownSnippet snippet = (KnownSnippet) element;
            int id = snippet.config.getId();
            if (id != configId && configId != -1) {
                return false; // snippets from different repos selected
            }
            configId = id;
            Optional<ISnippetRepository> repository = repos.getRepository(id);
            if (!repository.isPresent()) {
                return false;
            }
