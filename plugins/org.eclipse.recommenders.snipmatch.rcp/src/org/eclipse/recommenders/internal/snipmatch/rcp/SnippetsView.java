...
public class SnippetsView extends ViewPart implements IRcpService {
    ...
    private void shareSnippets() {
        ...
        Collection<ISnippet> snippets = Collections2.transform(selectedSnippets,
                new Function<KnownSnippet, ISnippet>() {
                    @Override
                    public ISnippet apply(KnownSnippet input) {
                        return input.snippet;
                    }
                });
        repository.get().share(snippets);
    }
