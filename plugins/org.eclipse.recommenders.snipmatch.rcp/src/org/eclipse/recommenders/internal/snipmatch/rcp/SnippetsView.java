...
public class SnippetsView extends ViewPart implements IRcpService {
    ...
    private void createActions(final Composite parent) {
        ...
        treeViewer.addSelectionChangedListener(new ISelectionChangedListener() {
            ...
            public void selectionChanged(SelectionChangedEvent event) {
                ...
                if (selectionConstistsOnlyElementsOf(SnippetRepositoryConfiguration.class)) {
                    removeRepoEnabled = true;
                }

                if (selectionConstistsOnlyElementsOf(KnownSnippet.class)) {
    ...
    private void removeRepos() {
        ensureIsTrue(selectionConstistsOnlyElementsOf(SnippetRepositoryConfiguration.class));
        ...
        for (SnippetRepositoryConfiguration config : configurations) {
            MessageDialogWithToggle confirmDialog = MessageDialogWithToggle.openOkCancelConfirm(tree.getShell(),
                    Messages.CONFIRM_DIALOG_DELETE_REPOSITORY_TITLE, Messages.CONFIRM_DIALOG_DELETE_REPOSITORY_MESSAGE,
    ...
    private void removeSnippets() {
        ensureIsTrue(selectionConstistsOnlyElementsOf(KnownSnippet.class));
    ...
    private void addContextMenu() {
        ...
        menuManager.addMenuListener(new IMenuListener() {
            ...
            public void menuAboutToShow(IMenuManager manager) {
                if (!selectionConstistsOnlyElementsOf(SnippetRepositoryConfiguration.class)
                        && !selectionConstistsOnlyElementsOf(KnownSnippet.class)) {
                    return;
                }
    ...
    protected Optional<SnippetRepositoryConfiguration> guessConfigurationForNewSnippet() {
        ...
        } else if (selectionConstistsOnlyElementsOf(KnownSnippet.class)) {
            List<KnownSnippet> selectedSnippets = castSelection();
    ...
    private <T> boolean selectionContainsOnlyOneElementOf(Class<T> aClass) {
        return selection.size() == 1 && aClass.isAssignableFrom(selection.get(0).getClass());
    }

    private <T> boolean selectionConstistsOnlyElementsOf(Class<T> aClass) {
