...
import org.eclipse.recommenders.snipmatch.GitSnippetRepository.GitBranchException;
                ...
                openJob = new Job(Messages.JOB_OPENING_SNIPPET_REPOSITORY) {
                    ...
                    protected IStatus run(IProgressMonitor monitor) {
                        try {
                            ...
                            return Status.OK_STATUS;
                        } catch (final GitBranchException e) {
                            changeStateToOpen();
                            Status status = new Status(IStatus.WARNING, Constants.BUNDLE_ID, MessageFormat.format(
                            ...
                            display.asyncExec(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    BranchCheckoutFailureDialog dialog = new BranchCheckoutFailureDialog(display
                                            .getActiveShell(), delegate
                                            .getRepositoryLocation(), Snippet.FORMAT_VERSION, e.getCheckoutVersion());
                                    dialog.open();
                                }
                            });
