...
public class EclipseGitSnippetRepository implements ISnippetRepository {
	...
	private static Logger LOG = LoggerFactory
			.getLogger(EclipseGitSnippetRepository.class);
	...
	public EclipseGitSnippetRepository(File basedir, String remoteUri,
			EventBus bus) {
		this.bus = bus;

		delegate = new GitSnippetRepository(new File(basedir,
				Urls.mangle(remoteUri)), remoteUri);
			...
			if (openJob == null && !delegateOpen) {
				openJob = new Job(Messages.JOB_OPENING_SNIPPET_REPOSITORY) {
					...
					protected IStatus run(IProgressMonitor monitor) {
						try {
							...
							Status status = new Status(
									...
									Constants.BUNDLE_ID,
									MessageFormat
											.format(Messages.WARNING_FAILURE_TO_UPDATE_REPOSITORY,
													delegate.getRepositoryLocation(),
													e.getMessage()), e);
							Platform.getLog(
									Platform.getBundle(Constants.BUNDLE_ID))
									.log(status);
							return Status.OK_STATUS;
						} catch (IOException e) {
							...
							Status status = new Status(
									...
									Constants.BUNDLE_ID,
									MessageFormat
											.format(Messages.ERROR_FAILURE_TO_CLONE_REPOSITORY,
													delegate.getRepositoryLocation(),
													e.getMessage()), e);
							Platform.getLog(
									Platform.getBundle(Constants.BUNDLE_ID))
					...
					private void changeStateToOpen() {
						delegateOpen = true;
						bus.post(new SnippetRepositoryOpenedEvent(
								EclipseGitSnippetRepository.this));
					}
				};
				openJob.schedule();
			}
	...
	/**
	 * Triggered when the snippet repository was closed to inform clients that
	 * the snippet repository is currently not available.
	 */
	...
	/**
	 * Triggered when the snippet repository was opened to inform clients that
	 * the snippet repository is available.
	 * <p>
	 * Clients of this event may consider refreshing themselves whenever they
	 * receive this event. Clients get notified in a background process.
	 */
	...
	public static ISnippetRepository createRepositoryInstance(
			EclipseGitSnippetRepositoryConfiguration config) {
		EventBus bus = InjectionService.getInstance().requestInstance(
				EventBus.class);
		File basedir = InjectionService.getInstance().requestAnnotatedInstance(
				File.class,
	...
	public static BasicEList<SnippetRepositoryConfiguration> getDefaultConfiguration() {
		...
		configuration
				.setDescription(Messages.ECLIPSE_GIT_SNIPPET_REPOSITORY_CONFIGURATION_DESCRIPTION);
		configuration.setEnabled(true);
		configuration
				.setUrl("https://git.eclipse.org/r/recommenders/org.eclipse.recommenders.snipmatch.snippets"); //$NON-NLS-1$
