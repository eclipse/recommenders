...
public abstract class MatchEnvironment {
    ...
    private final HashMap<ArgumentMatchNode, String[]> cachedArgumentCompletions;
     ...
     */
    public HashMap<String, String> getQueryTokenInterpretations(final String query) {

        final HashMap<String, String> interps = new HashMap<String, String>();
     ...
     * @throws Exception
     */
    public void applyMatch(final MatchNode match) throws Exception {

        // First, get the evaluated result of the match.
        final Object result = evaluateMatchNode(match);

        // Apply the result of the match to the environment.
        if (match instanceof EffectMatchNode) {

            final Effect effect = ((EffectMatchNode) match).getEffect();
     ...
     */
    public MatchNode[] getMatchCompletions(final MatchNode match) {

        final ArrayList<MatchNode> matchCompletions = new ArrayList<MatchNode>();

        final HashMap<ArgumentMatchNode, String[]> tokenCompletions = new HashMap<ArgumentMatchNode, String[]>();
        ...
        final Set<ArgumentMatchNode> keys = tokenCompletions.keySet();

        // Calculate the number of possible combinations of token completions.
        for (final ArgumentMatchNode argNode : keys) {
            numCompletions *= tokenCompletions.get(argNode).length;
        }

        // The indices indicating which completion should be used for each token.
        final HashMap<ArgumentMatchNode, Integer> completionIndices = new HashMap<ArgumentMatchNode, Integer>();
        ...
        for (int i = 0; i < numCompletions; i++) {
            ...
            final HashMap<ArgumentMatchNode, String> substitutions = new HashMap<ArgumentMatchNode, String>();
     ...
     */
    protected Object evaluateMatchNode(final MatchNode node) {

        /*
         * For argument nodes, simply return the argument. For effect nodes, use evaluateEffect and pass in the
         * arguments.
         */

        if (node instanceof EffectMatchNode) {

            final EffectMatchNode effectNode = (EffectMatchNode) node;

            final Object[] args = new Object[effectNode.getEffect().numParameters()];
            ...
            return ((ArgumentMatchNode) node).getArgument();
        }
    }
     ...
     * 
     * @param match
     *            The match for which to find token completions.
     * @param completions
     *            The map of token completions to modify.
     */
    private void collectArgumentCompletions(final MatchNode match,
            final HashMap<ArgumentMatchNode, String[]> completions) {

        if (match instanceof EffectMatchNode) {

            final EffectMatchNode effectNode = (EffectMatchNode) match;

            /*
             * The ghost child is used to make an entry in the completions for effects with no parameters. Because match
             * completions are generated from token completions, the ghost child is a way to make sure that effects with
             * no parameters are still considered when making match completions.
             */
            ...
            for (final MatchNode child : effectNode.getChildren()) {
     ...
     */
    private MatchNode completeMatch(final MatchNode match, final HashMap<ArgumentMatchNode, String> substitutions) {

        if (match instanceof EffectMatchNode) {

            final EffectMatchNode oldMatch = (EffectMatchNode) match;
            final ArrayList<MatchNode> newChildren = new ArrayList<MatchNode>();

            for (final MatchNode child : ((EffectMatchNode) match).getChildren()) {

                final MatchNode newChild = completeMatch(child, substitutions);
                newChildren.add(newChild);
            }

            final EffectMatchNode newMatch = new EffectMatchNode(oldMatch.getEffect(), oldMatch.getPattern(),
                    newChildren.toArray(new MatchNode[newChildren.size()]));
            ...
            final ArgumentMatchNode oldMatch = (ArgumentMatchNode) match;
            final ArgumentMatchNode newMatch = new ArgumentMatchNode(substitutions.get(oldMatch),
                    oldMatch.getParameter());
