...
public class SnippetParser {
     ...
     */
    public static ISnippetNode[] parseSnippetNodes(final Effect effect) {

        final ArrayList<ISnippetNode> nodes = new ArrayList<ISnippetNode>();

        StringBuilder sb = new StringBuilder();
        boolean inFormula = false;
        boolean hitDollar = false;

        for (final char c : effect.getCode().toCharArray()) {
     ...
     */
    private static FormulaSnippetNode parseFormulaSnippetNode(final String code, final Effect effect) {

        assert code.matches("\\A(\\w+:)?\\w+(\\((\\w+,)*\\w+\\))?\\Z");

        final int openPar = code.indexOf('(');
        final int closePar = code.indexOf(')');
        ...
        final FormulaSnippetNode node = new FormulaSnippetNode(name, args, effect);
        if (newVarName != null) {
