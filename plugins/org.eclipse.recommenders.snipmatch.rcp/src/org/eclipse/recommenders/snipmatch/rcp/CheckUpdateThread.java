...
public class CheckUpdateThread extends PostThread {
    ...
    public CheckUpdateThread(final SearchClient client) {
    ...
    public void run() {
        ...
        final Element featureNode = (Element) siteXml.getElementsByTagName("feature").item(0);
        final String version = featureNode.getAttribute("version");

        final int siteVersionInt = Integer.parseInt(version.replaceAll("\\.", ""));
        final int clientVersionInt = Integer.parseInt(client.getVersion().replaceAll("\\.", ""));
            ...
            PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
                ...
                public void run() {
                    ...
                    final MessageBox popup = new MessageBox(PlatformUI.getWorkbench().getActiveWorkbenchWindow()
                            .getShell(), SWT.ICON_INFORMATION | SWT.OK | SWT.APPLICATION_MODAL);

                    popup.setText("SnipMatch Update");
                    popup.setMessage("A new version of SnipMatch is available! "
                            + "Please go to Help > Check for Updates in the " + "Eclipse menu to obtain the update.");
                    popup.open();
                }
            });
        }
