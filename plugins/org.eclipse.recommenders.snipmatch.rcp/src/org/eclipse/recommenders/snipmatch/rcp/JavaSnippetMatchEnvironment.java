...
public class JavaSnippetMatchEnvironment extends SnippetMatchEnvironment {
    ...
    private final ArrayList<String> newImports;
    private final ArrayList<String[]> newHelpers;
    ...
    private final Hashtable<String, ArrayList<String>> compatibles;

    // code assist cache
    private final Hashtable<String, ArrayList<CompletionProposal>> codeAssistCache;

    public JavaSnippetMatchEnvironment() {

        // Get the active text editor.
        editor = (JavaEditor) PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().getActiveEditor();
        ...
        // Create a table of compatible primitive types. There should be a simpler way to check primitive type
        // compatibility.
    ...
    @Override
    public boolean testMatch(final MatchNode match) {

        /*
         * This method tests nested matches for type incompatibilities. Since nested results are not currently returned,
        ...
        if (match instanceof EffectMatchNode) {
            ...
            final Effect effect = effectMatch.getEffect();

            for (int i = 0; i < effectMatch.numChildren(); i++) {

                if (!testMatch(effectMatch.getChild(i))) {
                    return false;
                }

                final EffectParameter param = effect.getParameter(i);

                if (effectMatch.getChild(i) instanceof EffectMatchNode) {

                    final EffectMatchNode child = (EffectMatchNode) effectMatch.getChild(i);
                    final Effect childEffect = child.getEffect();

                    if (param.getMajorType().equals("expr")) {

                        if ((param.getMinorType() != null) && !param.getMinorType().isEmpty()) {

                            if ((childEffect.getMinorType() == null) || childEffect.getMinorType().isEmpty()) {
                                return false;
                            }

                            if (childEffect.getMinorType().equals(param.getMinorType())) {
                                return true;
                            }

                            if (param.getMinorType().equals("$array") || param.getMinorType().equals("[]")) {

                                if (childEffect.getMinorType().endsWith("[]")) {
                                    return true;
                                }
                            }
                            ...
                            if (param.getMinorType().equals("$loopable")) {
                                ...
                                try {
                                    ...
                                    final Class<?> parentClass = Iterable.class;
                                    final Class<?> childClass = Class.forName(childEffect.getMinorType(), false, null);
                            ...
                            for (final Entry<String, ArrayList<String>> entry : compatibles.entrySet()) {
                            ...
                            try {
                                ...
                                final Class<?> parentClass = Class.forName(param.getMinorType(), false, null);
                                final Class<?> childClass = Class.forName(childEffect.getMinorType(), false, null);
    ...
    public Point getSearchBoxAnchor(final int searchBoxHeight) {
        if (clean) {
            ...
            final int endOfSnippet = styledText.getLinePixel(lastLine + 1) + 2;
    ...
    public String[] getArgumentCompletions(final ArgumentMatchNode argNode) {
        ...
        final String arg = argNode.getArgument();
        ...
        try {
            ...
            final String argTest = "/**//**/";

            if (!codeAssistCache.containsKey(argTest)) {

                final ICompilationUnit cloneUnit = unit.getWorkingCopy(null);
                final int testOffset = (selOffset + argTest.length()) - 4;
            ...
            for (final CompletionProposal proposal : codeAssistCache.get(argTest)) {
        ...
        if (argNode.getParameter().getMajorType().equals("expr")) {
            ...
            try {
                ...
                /*
                 * If we have not already, perform a type-sensitive code completion on the argument. This works by
                 * temporarily inserting a dummy variable declaration into the code, and using code assist to see which
                 * values can be used to initialize it.
                 */
                ...
                if (!minorType.isEmpty() && !minorType.equals("$number") && !minorType.equals("$array")
                        && !minorType.equals("$loopable") && !minorType.equals("Object")) {
                ...
                cloneUnit.applyTextEdit(new ReplaceEdit(selOffset, selLength, argTest), null);
                final String contents = cloneUnit.getBuffer().getContents();
                ...
                for (final CompletionProposal proposal : codeAssistCache.get(argTest)) {
                    ...
                    if (proposal.getRelevance() < minRelevance) {
                        continue;
                    }

                    final String firstPart = contents.substring(testOffset - arg.length(), proposal.getReplaceStart());
                    final String secondPart = new String(proposal.getCompletion());
                    final String completion = firstPart + secondPart;

                    if (completion.equals(dummyName)) {
                        continue;
                    }
                    ...
                    case CompletionProposal.LOCAL_VARIABLE_REF:
                        ...
                        if (minorType.equals("$number")) {
                            if (!isNumericType(signature)) {
                                break;
                            }
                        } else if (minorType.equals("$array")) {
                            if (!isArrayType(signature)) {
                                break;
                            }
                        } else if (minorType.equals("$loopable")) {
                            if (!isLoopableType(signature)) {
                                break;
                            }
                        }
                    ...
                    case CompletionProposal.METHOD_REF:
                        ...
                        if (!argNode.getArgument().isEmpty() && signature.contains("()")) {
                            ...
                            final String returnType = signature.substring(signature.lastIndexOf(")") + 1);
        ...
        if (!exactMatch && !arg.endsWith(".")) {
            ...
            if (argNode.getParameter().getMajorType().equals("expr")) {
                ...
                if (minorType.equals("$number") || minorType.equals("int") || minorType.equals("long")
                        || minorType.equals("short") || minorType.equals("byte") || minorType.equals("char")
                    ...
                    if (arg.matches("[0-9]*") || arg.startsWith("\"")) {
                        completions.add(arg);
                    }
        ...
        /*
         * Make sure exactly one of the completions for each argument is blank. A blank argument means that a grayed out
         * parameter name will be shown instead.
         */
        completions.remove("");
        completions.add("");

        final String[] result = completions.toArray(new String[completions.size()]);
        return result;
    }
     ...
     * @return
     */
    private boolean isNumericType(final String typeName) {

        return typeName.equals("B") || typeName.equals("C") || typeName.equals("S") || typeName.equals("I")
                || typeName.equals("J") || typeName.equals("F") || typeName.equals("D");
    }
     ...
     * @return
     */
    private boolean isArrayType(final String typeName) {
    ...
    private boolean isLoopableType(String typeName) {
        ...
        try {
            ...
            final int angleStart = typeName.indexOf('<');
            if (angleStart != -1) {
                typeName = typeName.substring(0, angleStart);
            }

            final Class<?> parentClass = Iterable.class;
            final Class<?> childClass = Class.forName(typeName, false, null);
            return parentClass.isAssignableFrom(childClass);
        } catch (final ClassNotFoundException e) {
        }
    ...
    @Override
    protected String getQueryTokenInterpretation(final String token) {
     ...
     *            The match to apply.
     * @param confirmed
     *            False if this is just a code integration preview. True if this is a final integration.
     * @param searchBoxHeight
     *            The height of the search box.
     * @throws Exception
     */
    public void applyMatch(final MatchNode match, final boolean confirmed, final int searchBoxHeight) throws Exception {
     ...
     * @return
     */
    public Object evaluateMatchNodeOverview(final EffectMatchNode node) {
        String overViewText = (String) evaluateMatchNode(node);
        // Helper content, usually classes, ignore imports here
        String helper = null;
        final int start = node.getEffect().getCode().indexOf("${helper}");
        final int end = node.getEffect().getCode().indexOf("${endHelper}");
    ...
    @Override
    protected void applyResult(final Object result, final Effect effect) throws Exception {
        ...
        for (int i = 0; i < newHelpers.size(); i++) {
            ...
            final String[] helperInfo = newHelpers.get(i);
            final String helperCode = helperInfo[1];
            final String[] tokens = helperCode.split("\\s");
            ...
            for (int j = 0; j < tokens.length; j++) {
                ...
                if (tokens[j].equals("class")) {
                    ...
                    if (helperInfo[0] != null) {
                        // Replace all instances of the requested helper class name in the helper's code with the free
                        // class name.
        ...
        final int srcStartLine = workingDoc.getLineOfOffset(selOffset);
        final int srcEndLine = workingDoc.getLineOfOffset(selOffset + snippet.length());

        // Apply indentation to the lines of the inserted snippet.
        IndentUtil.indentLines(workingDoc, new LineRange(srcStartLine, (srcEndLine - srcStartLine) + 1), project, null);
        ...
        int srcEnd = workingDoc.getLineOffset(srcEndLine) + workingDoc.getLineLength(srcEndLine);

        final int destStartLine = doc.getLineOfOffset(selOffset);
        final int destEndLine = doc.getLineOfOffset(selOffset + selLength);

        int destStart = doc.getLineOffset(destStartLine);
        int destEnd = doc.getLineOffset(destEndLine) + doc.getLineLength(destEndLine);

        final String formattedCode = workingDoc.get(srcStart, srcEnd - srcStart);

        // This version of the text edit still contains the special markers, so we can keep working with them.
        final ReplaceEdit markerCodeEdit = new ReplaceEdit(destStart, destEnd - destStart, formattedCode);

        // This version of the text edit has the special markers removed, and is the one added to the final edit.
        final ReplaceEdit codeEdit = new ReplaceEdit(destStart, destEnd - destStart, formattedCode.replace(
                CURSOR_MARKER, "").replace(END_MARKER, ""));
        ...
        /*
         * The spacing of the document after editing Java elements varies based on whether or not a package statement
         * exists, so for consistency, we add a temporary dummy package statement if there isn't one.
         */
        ...
        if (confirmed) {
            ...
            for (final String[] helper : newHelpers) {

                if (helper[0] == null) {
                    continue;
                }

                // Auto-generate an ID comment to the helper class source, and make sure the class is not private or
                // public.
                String source = "// Snippet ID: " + effect.getId()
                        + helper[1].replaceAll("public class", "class").replaceAll("private class", "class");

                // We create a temporary document just to indent the helper class code properly.

                final IDocument helperIndentDoc = new Document(source);

                IndentUtil.indentLines(helperIndentDoc, new LineRange(0, helperIndentDoc.getNumberOfLines()), project,
                        null);
                ...
                try {
                    ...
                    final IType[] siblings = cloneUnit.getTypes();

                    if (siblings.length == 0) {
                        cloneUnit.createType(source, null, false, null);
                    } else {
                        cloneUnit.createType(source, siblings[0], false, null);
                    }
        ...
        if ((cloneUnit.getImports().length == 0) && (newImports.size() != 0)) {
            lastLine++;
        }

        // Update the lastLine variable based on the number of new import statements.
        for (final String newImport : newImports) {
        ...
        final IDocument importEditDoc = new Document(cloneUnit.getBuffer().getContents());

        int startLinesIn;

        for (startLinesIn = 0; startLinesIn < doc.getNumberOfLines(); startLinesIn++) {

            final String oldLine = doc.get(doc.getLineOffset(startLinesIn), doc.getLineLength(startLinesIn));

            final String newLine = importEditDoc.get(importEditDoc.getLineOffset(startLinesIn),
        ...
        for (endLinesIn = 0; endLinesIn < doc.getNumberOfLines(); endLinesIn++) {
            ...
            final String oldLine = doc.get(doc.getLineOffset(doc.getNumberOfLines() - endLinesIn - 1),
                    doc.getLineLength(doc.getNumberOfLines() - endLinesIn - 1));

            final String newLine = importEditDoc.get(
                    importEditDoc.getLineOffset(importEditDoc.getNumberOfLines() - endLinesIn - 1),
                    importEditDoc.getLineLength(importEditDoc.getNumberOfLines() - endLinesIn - 1));
        ...
        /*
         * Create another text edit that inserts all the helper classes and import statements, and add the edit to the
         * final edit.
         */
        if (startLinesIn <= (doc.getNumberOfLines() - endLinesIn)) {

            if (dummyPackage != null) {
                srcStart = importEditDoc.getLineOffset(startLinesIn + 1);
            } else {
                srcStart = importEditDoc.getLineOffset(startLinesIn);
            }
            ...
            final ReplaceEdit helperImportEdit = new ReplaceEdit(destStart, destEnd - destStart, importEditDoc.get(
                    srcStart, srcEnd - srcStart));
        ...
        /*
         * In the case where the user initiates a search near the end of a document, the text editor cannot scroll past
         * the end, causing problems for search box positioning. To fix this, we add temporary spaces to the end of the
         * document. This is only done during previews.
         */
        ...
        if (confirmed) {
            ...
            final int cursorMarkerPos = workingDoc.get().indexOf(CURSOR_MARKER);
            if (cursorMarkerPos != -1) {
                cursorPos = cursorMarkerPos;
            }

            // If no cursor position has been specified, use the end of the snippet.
            final int endMarkerPos = workingDoc.get().lastIndexOf(END_MARKER);
            if ((endMarkerPos != -1) && (cursorPos == -1)) {
                cursorPos = endMarkerPos;
            }
            ...
            final int resultsStart = doc.getLineOffset(lastLine);
            final int resultsEnd = doc.getLineOffset(lastLine
                    + (searchBoxHeight / (styledText.getLineHeight() + styledText.getLineSpacing())));

            // Direct manipulation of the scroll bar proved unsuccessful, so we select code to indirectly affect the
            // scroll bar.
        ...
        /*
         * Next, we access the text editor's projection annotation model, so we can take into account which methods /
         * classes have been folded. This makes the visual coordinates we get from the StyledText consistent with the
         * line offsets we get from the IDocument.
         */

        final ProjectionViewer viewer = (ProjectionViewer) editor.getViewer();
        final ProjectionAnnotationModel annotModel = viewer.getProjectionAnnotationModel();

        @SuppressWarnings("unchecked")
        final Iterator<ProjectionAnnotation> annots = annotModel.getAnnotationIterator();

        while (annots.hasNext()) {

            final ProjectionAnnotation annot = annots.next();
            final Position pos = annotModel.getPosition(annot);
            final int annotStartLine = doc.getLineOfOffset(pos.offset);
            final int annotEndLine = doc.getLineOfOffset(pos.offset + pos.length);
    ...
    @Override
    protected String evaluateFormulaSnippetNode(final FormulaSnippetNode formNode,
            final HashMap<String, String> variables, final EffectMatchNode effectNode) {
        ...
        if (formNode.numArguments() == 0) {
            ...
            final String varValue = variables.get(formNode.getName());
            ...
            if (varValue != null) {
                ...
                try {
                    ...
                    if (!codeAssistCache.containsKey(testString)) {
                        ...
                        cloneUnit.applyTextEdit(new ReplaceEdit(selOffset, selLength, testString), null);

                        cloneUnit.codeComplete((selOffset + testString.length()) - 4, new CompletionRequestor() {
                    ...
                    for (final CompletionProposal proposal : codeAssistCache.get(testString)) {
                ...
                while (true) {
                    ...
                    if (!variables.containsValue("" + iter) && !completions.contains("" + iter)) {
                        break;
                    }
        ...
        // Unary formulas...
            ...
            if (formNode.getName().equals("import")) {
                ...
                final String desiredName = formNode.getArgument(0);
                final ArrayList<String> completions = new ArrayList<String>();

                try {

                    final String testString = "/**/" + desiredName + "/**/";
                    ...
                    for (final CompletionProposal proposal : codeAssistCache.get(testString)) {
                        ...
                        case CompletionProposal.METHOD_REF:
                            ...
                            completions.add(completion.substring(0, completion.indexOf("(")));
                            break;
                        }
                    }
                } catch (final JavaModelException e) {
                ...
                while (true) {
                    ...
                    if (!variables.containsValue(currentName) && !completions.contains(currentName)) {
                        break;
                    }
    ...
    @Override
    protected String evaluateTextSnippetNode(final TextSnippetNode textNode, final HashMap<String, String> variables,
            final EffectMatchNode effectNode) {
    ...
    private String getFreeHelperName(final String desiredName, final String desiredId,
            final ICompilationUnit parentElement) throws Exception {
        ...
        while (!available) {
            ...
            for (final IType type : parentElement.getTypes()) {

                final String source = type.getSource();

                final int idStart = source.indexOf("// Snippet ID: ") + 15;
                final int idEnd = source.indexOf("\r", idStart);
                final int classStart = source.indexOf("class");

                if ((idStart < 0) || (idEnd < 0) || (classStart < idStart)) {
                    continue;
                }

                final String id = source.substring(idStart, idEnd);
    ...
    private String getVariableType(final String varName) {
        ...
        try {
            ...
            for (final CompletionProposal proposal : codeAssistCache.get(argTest)) {
                ...
                if (proposal.getSignature() != null) {
                    ...
                    final int argListEnd = signature.indexOf(')');
                    if (argListEnd != -1) {
                        signature = signature.substring(argListEnd + 1);
                    }
                    ...
                    if (signature.startsWith("L") && signature.endsWith(";")) {
                        ...
                        case 'Z':
                            finalType = "boolean" + finalType;
                            break;
                        case 'B':
                            finalType = "byte" + finalType;
                            break;
                        case 'C':
                            finalType = "char" + finalType;
                            break;
                        case 'S':
                            finalType = "short" + finalType;
                            break;
                        case 'I':
                            finalType = "int" + finalType;
                            break;
                        case 'J':
                            finalType = "long" + finalType;
                            break;
                        case 'F':
                            finalType = "float" + finalType;
                            break;
                        case 'D':
                            finalType = "double" + finalType;
                            break;
                        }
        ...
        if (varTypes.get(0).equals("java.lang.String")) {
            return "String";
        } else if (varTypes.get(0).equals("java.lang.Object")) {
            return "Object";
        } else {
            return varTypes.get(0);
        }
    }
    ...
    private String getElementType(final String loopableName) {
        ...
        try {
            ...
            final String argTest = "/**/" + loopableName + "/**/";
            ...
            for (final CompletionProposal proposal : codeAssistCache.get(argTest)) {
                ...
                if (proposal.getRelevance() < 25) {
                    continue;
                }

                final String completion = new String(proposal.getCompletion());
                if (!completion.equals(loopableName)) {
                    continue;
                }
                ...
                if (proposal.getSignature() != null) {
                    ...
                    if (typeStart == -1) {
                        typeStart = signature.indexOf("<L");
                    }

                    if (typeStart != -1) {
                        final int typeEnd = signature.indexOf(";", typeStart);
        ...
        if (elemTypes.isEmpty()) {
            elemTypes.add("java.lang.Object");
        }
        return getPracticalTypeName(elemTypes.get(0));
    }

    private String getPracticalTypeName(final String canonicalTypeName) {

        if (canonicalTypeName.endsWith("[]")) {
            return getPracticalTypeName(canonicalTypeName.substring(0, canonicalTypeName.lastIndexOf("["))) + "[]";
        }

        final int leftAngle = canonicalTypeName.indexOf("<");
        final int rightAngle = canonicalTypeName.lastIndexOf(">");
