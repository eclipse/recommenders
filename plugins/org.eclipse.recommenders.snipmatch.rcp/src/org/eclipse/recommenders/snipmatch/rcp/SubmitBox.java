...
public class SubmitBox extends ClientSwitcher{
	...
	public void show(Effect toEdit) {
		...
		envs = new MatchEnvironment[] {
				new JavaSnippetMatchEnvironment()
		};

		editing = toEdit != null;

		if (editing) effect = toEdit;
		else {
			effect = new Effect();
		...
		if (editing) shell.setText("Edit Snippet");
		else shell.setText("Submit Snippet");
...

		{
			...
			
			{
				...
				codeText = new StyledText(page, SWT.BORDER | SWT.MULTI |
...

		{
			...
			
			{
				...
				publicText.setText("Allow others to use this snippet.\n" +
						"Snippet will be subject to approval before being made visible to others.");
			...
			
			{
				...
				publicText = new Text(page, SWT.MULTI | SWT.WRAP);
				publicText.setText("By checking the box above, you confirm that you have " +
						"full rights to release the code in this snippet, and agree to " +
						"release it to the public domain. You also understand that " +
						"Language Interfaces is not responsible for any misuse of this code.");
				publicText.setEditable(false);
				publicText.setLayoutData(fd);
			}
		}
		
		{
			...
			link.setText("<a href=\"" + SnipMatchPlugin.SUBMIT_HELP_URL +
	...
	private void showEditParameterDialog() {
		...
		
		{
			...
			nameText = new Text(dialog, SWT.BORDER);
			nameText.setText(effect.getParameter(
					paramsList.getSelectionIndex()).getName());
			nameText.setLayoutData(fd);
		}
		
		{
			...
			minorTypeText = new Text(dialog, SWT.BORDER);
			minorTypeText.setText(effect.getParameter(
					paramsList.getSelectionIndex()).getMinorType());
			minorTypeText.setLayoutData(fd);
		...
		
		{
			...
			okButton.addSelectionListener(new SelectionListener() {
				...
				public void editParameter() {
					...
					EffectParameter param = effect.getParameter(
							paramsList.getSelectionIndex());
					
					param.setName(name);
					param.setMajorType(majorType);
					param.setMinorType(minorType);
					
					paramsList.setItem(paramsList.getSelectionIndex(),
							name + " (" + minorType + ")");
	...
	private boolean checkPattern(String pattern) {
		...
		for (int i = 0; i < effect.numPatterns(); i++) {
			...
			if (otherPattern.equals(pattern)) numEqual++;
	...
	private void submitEffect() {
		...
		for (int i = 0; i < effect.numPatterns(); i++) {
			if (!checkPattern(effect.getPattern(i))) return;
		}

		// Check each parameter for errors.
		for (int i = 0; i < effect.numParameters(); i++) {
			if (!checkParameter(effect.getParameter(i))) return;
		}
		...
		if (editing) {
			...
			client.startDeleteEffect(effect, new IDeleteEffectListener() {
				...
				public void deleteEffectSucceeded() {
					...
					client.startSubmitEffect(effect, isPublic,
							new ISubmitEffectListener() {
			...
			client.startSubmitEffect(effect, publicButton.getSelection(),
					new ISubmitEffectListener() {
	...
	private ArrayList<String> getParameterTokens(String pattern) {
		...
		for (String token : pattern.split(" ")) {
			if (token.startsWith("$")) tokens.add(token.substring(1));
		}
