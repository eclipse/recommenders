...
public class StringCompareSearchEngine implements SnipMatchSearchEngine {
	...
	public void initialize(String snippetsDir, String indexDir) throws IOException {
		...
		String indexFilePath = SnipMatchPlugin.getDefault().getPreferenceStore().getString(PreferenceConstants.SNIPPETS_INDEX_FILE);
	...
	public void updateIndex() {
		if(PlatformUI.getWorkbench().getActiveWorkbenchWindow() != null){
			...
			try {
				...
				e.printStackTrace();
				MessageDialog.openError(shell, "Recommenders tips", "Fail to upate index, check your snippet store directory!");
			} 
		}
	}
...
class CreateIndexOperation implements IRunnableWithProgress{
	private String indexFilePath = SnipMatchPlugin.getDefault().getPreferenceStore().getString(PreferenceConstants.SNIPPETS_INDEX_FILE);
	private String dirPath = SnipMatchPlugin.getDefault().getPreferenceStore().getString(PreferenceConstants.SNIPPETS_STORE_DIR);
	@Override
