...
public class PoolingStaticsModelProvider extends PoolingModelProvider<UniqueTypeName, IStaticModel>
        implements IStaticModelProvider {
    ...
    public PoolingStaticsModelProvider(IModelRepository repo, IModelArchiveCoordinateAdvisor index,
        ...
        super(repo, index, "statics", transformers);
    }

    @Override
    protected void passivateModel(IStaticModel model) {
        model.reset();
    }

    @Override
    protected IStaticModel loadModel(InputStream in, UniqueTypeName key) throws IOException {
        return JayesStaticModel.load(in, key.getName());
    }
