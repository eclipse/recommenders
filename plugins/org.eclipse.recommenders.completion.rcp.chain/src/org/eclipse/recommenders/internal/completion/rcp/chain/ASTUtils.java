...
public class ASTUtils {
    ...
    // TODO: This is primitive. Use binding for expectedType and then do returnType.isCompatibleWith(expectedType).
    public static boolean isAssignable(final IType expectedType, final TypeBinding returnType) {
        ensureIsNotNull(expectedType);
        ensureIsNotNull(returnType);
        final String key = expectedType.getKey();
        for (final ReferenceBinding supertype : findAllSupertypesIncludeingArgument(returnType)) {
            if (key.equals(String.valueOf(supertype.signature()))) {
                return true;
            }
        }
        return false;
    ...
    private static List<ReferenceBinding> findAllSupertypesIncludeingArgument(final TypeBinding returnType) {
        // TODO: I don't think should happen, but it does, e.g. with array return types. Needs another strategy!
        if (!(returnType instanceof ReferenceBinding)) {
            return Collections.emptyList();
        }
        final List<ReferenceBinding> supertypes = new LinkedList<ReferenceBinding>();
        ReferenceBinding superclass = (ReferenceBinding) returnType;
