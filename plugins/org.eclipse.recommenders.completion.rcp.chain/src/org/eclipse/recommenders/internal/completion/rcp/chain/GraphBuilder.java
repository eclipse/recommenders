...
public class GraphBuilder {
    ...
    private static final int maxchains = 2000;
    ...
    private final Table<IType, IType, Boolean> assignableCache = HashBasedTable.create();
    ...
    void startChainSearch(final List<MemberEdge> entrypoints, final IType expectedType) {
        ...
        while (!incompleteChains.isEmpty()) {
            ...
            if (isAssignableTo(returnType, expectedType)) {
            ...
            final Collection<IMember> allMethodsAndFields = findAllFieldsAndMethods(returnType);
    ...
    private boolean isAssignableTo(final IType returnType, final IType expectedType) {
        Boolean cached = assignableCache.get(returnType, expectedType);
        if (cached == null) {
            cached = JdtUtils.isAssignable(expectedType, returnType);
            assignableCache.put(returnType, expectedType, cached);
        }
        return cached;
    }
