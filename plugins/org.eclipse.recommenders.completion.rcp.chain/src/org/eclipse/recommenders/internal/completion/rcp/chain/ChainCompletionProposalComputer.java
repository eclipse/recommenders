...
public class ChainCompletionProposalComputer implements IJavaCompletionProposalComputer {
    ...
    private void addPublicStaticMembersToEntrypoints(final TypeBinding type) {
        for (final Binding m : TypeBindingAnalyzer.findAllPublicStaticFieldsAndNonVoidNonPrimitiveStaticMethods(type,
                enclosingType)) {
    ...
    private void addPublicInstanceMembersToEntrypoints(final TypeBinding type) {
        for (final Binding m : TypeBindingAnalyzer.findVisibleInstanceFieldsAndRelevantInstanceMethods(type,
                enclosingType)) {
            if (matchesExpectedPrefix(m)) {
    ...
    private List<ICompletionProposal> executeCallChainSearch() {
        final TypeBinding expectedType = Preconditions.checkNotNull(TypeBindingHack.resolveBindingForExpectedType(ctx));
        final ChainFinder b = new ChainFinder(expectedType);
        try {
            new SimpleTimeLimiter().callWithTimeout(new Callable<Void>() {

                @Override
                public Void call() throws Exception {
                    b.startChainSearch(enclosingType, entrypoints, prefStore.getInt(ChainPreferencePage.ID_MAX_CHAINS),
