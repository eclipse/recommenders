...
public class ChainCompletionProposalComputer implements IJavaCompletionProposalComputer {
    ...
    private boolean findEntrypoints() {
        ...
        // TODO: Find IType instead.
        enclosingType = ctx.getClosestEnclosingType().orNull();
        if (enclosingType == null) {
            RecommendersUtilsPlugin.logWarning("Can't find enclosing type for completion node %s", node);
            return false;
        }
    ...
    private void addPublicStaticMembersToEntrypoints(final TypeBinding type) {
        for (final Binding m : findAllPublicStaticFieldsAndNonVoidNonPrimitiveStaticMethods(type, enclosingType)) {
            if (matchesExpectedPrefix(m)) {
                final ChainElement edge = new ChainElement(m, false);
    ...
    // TODO: Core context seems to ignore fields once a local variable of same name is visible.
    ...
    private Set<String> getLocalVariableNames(final ObjectVector visibleLocalVariables) {
    ...
    private List<ICompletionProposal> executeCallChainSearch() {
        ...
        final ChainFinder finder = new ChainFinder(expectedType.get());
        try {
