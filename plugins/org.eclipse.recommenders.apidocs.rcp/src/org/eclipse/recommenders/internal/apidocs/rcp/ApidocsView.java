...
import static org.eclipse.recommenders.internal.apidocs.rcp.ApidocsViewHelpers.*;
...
import org.eclipse.recommenders.internal.rcp.Activator;
import org.eclipse.recommenders.rcp.JavaSelectionEvent;
...
public class ApidocsView extends ViewPart {
    ...
    private void createProviderOverview() {
        ...
        viewer.setComparator(new ViewerComparator() {
            ...
            public int compare(Viewer viewer, Object first, Object second) {
                int indexFirst = providerRanking.indexOf((ApidocProvider) first);
                int indexSecond = providerRanking.indexOf((ApidocProvider) second);
    ...
    private void addDnDSupport() {
        ...
        viewer.addDropSupport(operations, transferTypes, new ViewerDropAdapter(viewer) {
            ...
            private boolean isFirstProvider(final DropTargetEvent event) {
                return providerRanking.indexOf((ApidocProvider) determineTarget(event)) == 0;
            }

            @Override
            public void drop(final DropTargetEvent event) {
                if (event.item != null) {
                    newIndex = providerRanking.indexOf((ApidocProvider) event.item.getData());
                } else {
    ...
    private void createContentArea() {
        ...
        ApidocsViewHelpers.setInfoBackgroundColor(content);
    ...
    public void onJavaSelection(final JavaSelectionEvent selection) {
        ...
        if (visible && activeProvider != null && activeSelection != null) {
            try {
                ...
                Activator.logError(e, "Exception during view update." + selection); //$NON-NLS-1$
            }
        }
    }
    ...
    private void disposeOldContentAndDisableRedrawOnContentArea() {
        content.getDisplay().syncExec(new Runnable() {
            ...
            public void run() {
                ...
                ApidocsViewHelpers.disposeChildren(content);
    ...
    private void addCurrentSelectionHeader() {
        ...
        ApidocsViewHelpers.setInfoBackgroundColor(header);
