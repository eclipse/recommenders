...
import static org.eclipse.recommenders.rcp.JavaElementSelectionEvent.JavaSelectionLocation.METHOD_DECLARATION;
...
public class StaticHooksFinder extends ApidocProvider {
    ...
    public void onPackageRootSelection(final IPackageFragmentRoot root, final JavaElementSelectionEvent event,
        ...
        try {
            ...
            Activator.logError(x, "Failed to determine static members for %s", root.getElementName()); //$NON-NLS-1$
        }
        runSyncInUiThread(new HooksRendererRunnable(index, parent));
    }

    @JavaSelectionSubscriber
    public void onPackageSelection(final IPackageFragment pkg, final JavaElementSelectionEvent event, final Composite parent)
        ...
        try {
            ...
            Activator.logError(e, "Failed to determine static members for package %s", pkg.getElementName()); //$NON-NLS-1$
        }
