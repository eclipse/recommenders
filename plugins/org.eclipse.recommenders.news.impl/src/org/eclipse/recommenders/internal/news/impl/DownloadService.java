...
public class DownloadService implements IDownloadService {
    ...
    @VisibleForTesting
    DownloadService(Path downloadLocation) {
        this.downloadLocation = downloadLocation;
    }

    private static Path getStateLocation() {
        Bundle bundle = FrameworkUtil.getBundle(DownloadService.class);
    ...
    private void doDownload(URI uri, String fileName, Path targetFile, IProgressMonitor monitor) throws IOException {
    ...
    private InputStream openWebResourceAsStream(URI uri, IProgressMonitor monitor) throws IOException {
        SubMonitor progress = SubMonitor.convert(monitor, 1);
        try {
            Request request = Request.Get(uri).viaProxy(Proxies.getProxyHost(uri).orNull())
