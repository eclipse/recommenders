...
import org.eclipse.recommenders.statics.IStaticModel;
import org.eclipse.recommenders.statics.IStaticModelProvider;
...
public class StaticsCompletionSessionProcessor extends SessionProcessor {
    ...
    private DecimalFormat permilFormatter = new DecimalFormat("0.00%");
    ...
    private IStaticModelProvider modelProvider;

    private Map<IMethodName, Double> recommendations;
    private StaticsRcpPreferences prefs;

    @Inject
    public StaticsCompletionSessionProcessor(Provider<IProjectCoordinateProvider> pcProvider,
            IStaticModelProvider modelProvider, IProposalNameProvider methodNameProvider, SharedImages images,
    ...
    public boolean startSession(final IRecommendersCompletionContext context) {
        ...
        for (CompletionProposal p : context.getProposals().values()) {
            if (!isStaticMethodCallInternalCompletionProposal(p)) {
                continue;
            }
            InternalCompletionProposal internal = (InternalCompletionProposal) p;
            Binding binding = internal.getBinding();
            ...
            proposals.put(methodBinding.declaringClass, p);
        }

        for (TypeBinding typeBinding : proposals.keySet()) {
            // IType type = project.findType(sType);
            IType type = JdtUtils.createUnresolvedType(typeBinding).orNull();
            if (type == null) {
                continue;
            }
            Result<UniqueTypeName> res = pcProvider.get().tryToUniqueName(type);
            if (!res.isPresent()) {
                continue;
            }
            UniqueTypeName recType = res.get();
            IStaticModel model = modelProvider.acquireModel(recType).orNull();
            if (model == null) {
                continue;
            }
            IJavaElement enclosingMethod = ctx.getEnclosingElement().orNull();
            if (enclosingMethod != null) {
                switch (enclosingMethod.getElementType()) {
                case IJavaElement.METHOD:
                    IMethodName enclosignMethodName = pcProvider.get().toName((IMethod) enclosingMethod).orNull();
                    model.setEnclosingMethod(enclosignMethodName);
                    break;
                case IJavaElement.INITIALIZER:
                    try {
                        int flags = ((IInitializer) enclosingMethod).getFlags();
                        enclosignMethodName = VmMethodName.get("Lsome",
