...
public class ProjectCoordinatesView extends ViewPart {
    ...
    private ManualMappingStrategy manualMappingStrategy;
    ...
    @Inject
    public ProjectCoordinatesView(final EclipseDependencyListener eclipseDependencyListener,
            final IMappingProvider mappingProvider, final ManualMappingStrategy manualMappingStrategy) {
        this.eclipseDependencyListener = eclipseDependencyListener;
        this.mappingProvider = mappingProvider;
        this.manualMappingStrategy = manualMappingStrategy;
    }
            ...
            try {
                PlatformUI.getWorkbench().getActiveWorkbenchWindow().run(true, false, new IRunnableWithProgress() {
                    public void run(IProgressMonitor monitor) {
                        ...
                        strategies = mappingProvider.getStrategies();
                        for (DependencyInfo dependency : dependencyInfos) {
                            ...
                            for (IProjectCoordinateResolver strategy : strategies) {
                                data.put(dependency, strategy.searchForProjectCoordinate(dependency));
                            }
                            monitor.worked(1);
                        }
                    }
    ...
    private void addFilterFunctionality() {
        final ViewerFilter manualMappingsFilter = new ViewerFilter() {
            ...
            private boolean isManualMapping(Entry<DependencyInfo, Collection<Optional<ProjectCoordinate>>> entry) {
                int indexOfManualMapping = mappingProvider.getStrategies().indexOf(manualMappingStrategy);
                int i = 0;
                    ...
                    for (Optional<ProjectCoordinate> optionalCoordinate : entry.getValue()) {
                        if (optionalCoordinate.isPresent()) {
                            if (coordinate == null) {
                                ...
                                if (!coordinate.equals(optionalCoordinate.get())) {
                                    return true;
                                }
                            }
                        }
                    ...
                    if (element instanceof Item) {
                        ...
                        if (data instanceof Entry) {
                            ...
                            if ("".equals((String) value)) {
                                manualMappingStrategy.removeManualMapping(entry.getKey());
                            } else {
                                try {
                                    ProjectCoordinate valueOf = ProjectCoordinate.valueOf((String) value);
                                    manualMappingStrategy.setManualMapping(entry.getKey(), valueOf);
                                } catch (Exception e) {
    ...
    class CoordinateTooltip extends ToolTipProvider {
        ...
        protected String generateTooltip(final Entry<DependencyInfo, Collection<Optional<ProjectCoordinate>>> entry) {
            DependencyInfo dependencyInfo = entry.getKey();
            StringBuilder sb = new StringBuilder();
            List<IProjectCoordinateResolver> strategies = contentProvider.getStrategies();
            List<Optional<ProjectCoordinate>> coordinates = Lists.newArrayList(entry.getValue());

            for (int i = 0; i < strategies.size(); i++) {
                IProjectCoordinateResolver strategy = strategies.get(i);
