...
public class DownloadModelArchiveJob extends Job {
    ...
    protected IStatus run(final IProgressMonitor monitor) {
        try {
            ...
            ModelArchiveDownloadCallback cb = new ModelArchiveDownloadCallback(monitor, TOTAL_WORK_UNITS);
            File result = repository.resolve(mc, forceDownload, cb).orNull();
            if (cb.isDownloadedArchive()) {
                bus.post(new ModelArchiveDownloadedEvent(mc));
            }

            cb.updateMonitorWorkForSkippedDownloadTasks();
