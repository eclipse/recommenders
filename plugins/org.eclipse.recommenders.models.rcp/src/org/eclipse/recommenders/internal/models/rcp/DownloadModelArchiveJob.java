...
public class DownloadModelArchiveJob extends Job {
    ...
    private static final int MAXIMUM_AMOUNT_OF_DOWNLOADS_PER_JOB = 2;
    ...
    protected IStatus run(final IProgressMonitor monitor) {
        try {
            ...
            ModelArchiveDownloadCallback downloadCallback = new ModelArchiveDownloadCallback(monitor, TOTAL_WORK_UNITS,
                    MAXIMUM_AMOUNT_OF_DOWNLOADS_PER_JOB);
            File result = repository.resolve(mc, forceDownload, downloadCallback).orNull();
            boolean archiveDownloaded = downloadCallback.isDownloadSucceeded();
            if (archiveDownloaded) {
                bus.post(new ModelArchiveDownloadedEvent(mc));
            }
            downloadCallback.allDownloadsFinished();
