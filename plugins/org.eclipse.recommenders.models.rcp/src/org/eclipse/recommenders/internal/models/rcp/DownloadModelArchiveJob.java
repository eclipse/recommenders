...
public class DownloadModelArchiveJob extends Job {
    ...
    private ModelArchiveCoordinate model;

    public DownloadModelArchiveJob(IModelRepository repository, ModelArchiveCoordinate model) {
        super(String.format(JOB_RESOLVING_MODEL, model));
    ...
    protected IStatus run(final IProgressMonitor monitor) {
        try {
            repository.resolve(model, new DownloadCallback() {
                @Override
            ...
            return newStatus(BUNDLE_ID, "failed to download " + model, e);
        } finally {
            monitor.done();
        }
        return OK_STATUS;
