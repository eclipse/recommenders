...
public class RepositoryDetailsDialog extends TitleAreaDialog {
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        String title = noRepository ? "Add Repository" : "Edit Repository";

        getShell().setText(title);
        setTitle(title);

        setMessage(noRepository ? "Enter repository details." : "Edit repository details.");
        ...
        repositoryUrlText = createLabelledText(fieldArea, "Repository URL:",
                noRepository ? "http://download.eclipse.org/recommenders/models/<version>" : repositoryUrl, SWT.NONE);

        repositoryUsernameText = createLabelledText(fieldArea, "Username:",
                preferences.getServerUsername(repositoryUrl).or(""), SWT.NONE); //$NON-NLS-1$
        repositoryUsernameText.setMessage("Optional");

        repositoryPasswordText = createLabelledText(fieldArea, "Password:", //$NON-NLS-1$
                preferences.getServerPassword(repositoryUrl).or(""), SWT.PASSWORD);

        repositoryPasswordText.setMessage("Optional");
    ...
    private void updatePageComplete() {
        ...
        try {
            ...
            if (!isUsernameValid()) {
                hasError = true;
                setErrorMessage("You must specify a username if a password is set.");
                return;
            }
        } finally {
    ...
    private final class ServerRepositoryUrlListener implements ModifyListener {
        ...
        public void modifyText(ModifyEvent e) {
            ...
            String password = preferences.getServerPassword(serverUri).or(""); // Can't set null password
