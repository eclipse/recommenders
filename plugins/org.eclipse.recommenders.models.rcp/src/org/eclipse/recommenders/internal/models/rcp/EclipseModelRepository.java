...
public class EclipseModelRepository implements IModelRepository, IRcpService {
    ...
    private List<ModelRepository> searchDelegates(ModelCoordinate mc) {
        Optional<String> repositoryHint = mc.getHint(ModelCoordinate.REPOSITORY_URL);
        if (repositoryHint.isPresent()) {
            ModelRepository modelRepository = delegates.get(repositoryHint.get());
