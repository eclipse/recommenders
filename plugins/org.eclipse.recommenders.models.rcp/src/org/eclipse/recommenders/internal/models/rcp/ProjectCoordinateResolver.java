...
public class ProjectCoordinateResolver implements org.eclipse.recommenders.models.rcp.IProjectCoordinateResolver,
        IRcpService {
    ...
    @Inject
    public ProjectCoordinateResolver(@Named(IDENTIFIED_PACKAGE_FRAGMENT_ROOTS) File persistenceFile,
    ...
    @Override
    public Optional<QualifiedTypeName> toQualifiedName(IType type) {
        ProjectCoordinate base = resolve(type).orNull();
        if (null == base) {
            return absent();
        }
        return of(new QualifiedTypeName(base, toName(type)));
    }
