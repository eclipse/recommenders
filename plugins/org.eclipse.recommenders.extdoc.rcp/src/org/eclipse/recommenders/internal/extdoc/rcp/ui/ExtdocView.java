...
public class ExtdocView extends ViewPart {
	...
	viewer.addDragSupport(operations, transferTypes,
		...
		if (ExtdocProviderTransfer.getInstance()
			.isSupportedType(event.dataType)) {
		    final ExtdocProvider selectedProvider = (ExtdocProvider) selection
			    .getFirstElement();
		    ExtdocProviderTransfer.getInstance()
		    .setExtdocProvider(selectedProvider);
		}
	    }
	...
	viewer.addDropSupport(operations, transferTypes, 
		...
		if (newIndex != oldIndex) {
		    final ExtdocProvider movedProvider = providers
			    .remove(oldIndex);
