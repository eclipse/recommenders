...
import static org.eclipse.recommenders.internal.completion.rcp.Constants.SUBWORDS_COMPLETION_PREFERENCE_PAGE_ID;
import static org.eclipse.recommenders.internal.subwords.rcp.Constants.SESSION_PROCESSOR_ID;
...
import org.eclipse.recommenders.internal.completion.rcp.ICompletionTipProposal;
...
public class EnableSubwordsCompletionProposal extends AbstractJavaCompletionProposal implements ICompletionTipProposal {
    ...
    public boolean isApplicable() {
        for (SessionProcessorDescriptor descriptor : preferences.getSessionProcessors()) {
            if (descriptor.getId().equals(Constants.SESSION_PROCESSOR_ID)) {
    ...
    private void enableSubwordsCompletion() {
        for (SessionProcessorDescriptor descriptor : preferences.getSessionProcessors()) {
            if (descriptor.getId().equals(SESSION_PROCESSOR_ID)) {
                descriptor.setEnabled(true);
                preferences.storeSessionProcessors();
                return;
            }
        }
    }
