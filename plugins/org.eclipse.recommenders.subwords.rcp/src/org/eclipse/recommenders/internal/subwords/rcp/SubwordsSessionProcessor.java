...
public class SubwordsSessionProcessor extends SessionProcessor {
    ...
    public void process(final IProcessableProposal proposal) {
        ...
        System.err.println(proposal.getCoreProposal().orNull());
        System.err.println(completionIdentifier);
        System.err.println(matchingArea);
        System.err.println();
            ...
            public boolean isPrefix(String prefix) {
                if (this.prefix != prefix) {
                    ...
                    if (lastIndexOfHash >= 0) {
                        // This covers the case where the user starts with a prefix of "Collections#" and continues from there.
                        bestSequence = LCSS.bestSubsequence(matchingArea,
                                lastIndexOfHash < 0 ? prefix : prefix.substring(lastIndexOfHash + 1));
                    } else {
                        // Besides the obvious, this also covers the case where the user starts with a prefix of "Collections#e".
