...
public class SubwordsSessionProcessor extends SessionProcessor {
    ...
    public void process(final IProcessableProposal proposal) {
        ...
        proposal.getProposalProcessorManager().addProcessor(new ProposalProcessor() {
            ...
            /**
             * The fundamental logic of this method has been taken from the method computeRelevanceForCaseMatching() of
             * {@link org.eclipse.jdt.internal.codeassist.CompletionEngine}
             */
            @Override
            public int modifyRelevance() {
                ...
                /*
                 * Since we may simulate completion triggers at positions before the actual triggering, we don't get
                 * JDT's additional relevance for exact prefix matches. So we add the additional relevance ourselves, if
                 * is not already supplied by the JDT which it does, if the prefix is shorter than the configured
                 * minimum prefix length. (see Bug 468494)
                 *
                 * The boost is the same one as JDT adds at {@link
                 * org.eclipse.jdt.internal.codeassist.CompletionEngine#computeRelevanceForCaseMatching}
                 *
                 * The boost is further multiplied by 16 which reflects the same thing happening in {@link
