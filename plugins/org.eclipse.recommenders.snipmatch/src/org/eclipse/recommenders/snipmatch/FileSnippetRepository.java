...
public class FileSnippetRepository implements ISnippetRepository {
    ...
    private Map<File, Float> searchSnippetFiles(ISearchContext context, int maxResults) {
        ...
        try {
            ...
            for (ScoreDoc hit : searcher.search(q, null, maxResults).scoreDocs) {
                ...
                if (!isSnippetApplicable(doc, context)) {
                    continue;
                }
    ...
    private boolean isSnippetApplicable(Document doc, ISearchContext context) {
        ...
        for (String snippetDependency : snippetDependencies) {
            ...
            for (ProjectCoordinate workspaceDependency : context.getDependencies()) {
                if (isApplicable(workspaceDependency, snippetDependency)) {
                    applicable = true;
                    break;
                }
            }
    ...
    private boolean isApplicable(ProjectCoordinate pc, String dependency) {
