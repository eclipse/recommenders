...
public class GitSnippetRepository extends FileSnippetRepository {
        ...
        synchronized (this) {
            ...
            try {
                ...
                throw createException(gitFileExists,
                        String.format("Invalid remote repository '%s'. Check the repository's URL.", repoUrl), e);
                ...
                throw createException(gitFileExists, "Exception while update/clone repository.", e);
    ...
    public class GitUpdateException extends IOException {
        public GitUpdateException(String message, Throwable cause) {
            super(MessageFormat.format("{0} {1}", message, "You are working with a previously downloaded version that may not be up to date."), cause); //$NON-NLS-1$
        }
    }
