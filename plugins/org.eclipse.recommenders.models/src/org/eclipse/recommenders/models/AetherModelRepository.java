...
public class AetherModelRepository implements IModelRepository {
	...
	private ListeningExecutorService executor = MoreExecutors
			.listeningDecorator(Executors.coreThreadsTimoutExecutor(1,
					Thread.MIN_PRIORITY, "model-downloader"));

	private Artifact INDEX = new DefaultArtifact("org.eclipse.recommenders",
			"index", "zip", "0.0.0");
	...
	public void open() throws Exception {
		if (doesNotExistOrIsAlmostEmptyFolder(indexdir)) {
			File zippedIndex = executor.submit(
					new DownloadArtifactTask(INDEX, null)).get();
	...
	private boolean doesNotExistOrIsAlmostEmptyFolder(File location) {
		...
		// 2 = if this folder contains an index, there must be more than one
		// file...
		// on mac, we often have hidden files in the folder. This is just a
		// simple heuristic.
	}
	...
	public ListenableFuture<File> schedule(final ModelArchiveCoordinate model,
		...
		final DownloadCallback cb = firstNonNull(callback,
				DownloadCallback.NULL);
		final Artifact coord = new DefaultArtifact(model.getGroupId(),
				model.getArtifactId(), model.getClassifier(),
				model.getExtension(), model.getVersion());
		return executor.submit(new DownloadArtifactTask(coord,
	...
	private String computePath(ModelArchiveCoordinate artifact) {
		...
		sb.append(groupId).append('/').append(artifactId).append('/')
				.append(version).append('/').append(artifactId).append('-')
				.append(version);
	...
	public ModelArchiveCoordinate[] findModelArchives(
			ProjectCoordinate projectCoord, String modelType) {
		...
		for (String model : findByTerm(modelType, t1, t2)) {
			...
			results.add(new ModelArchiveCoordinate(tmp.getGroupId(), tmp
					.getArtifactId(), tmp.getClassifier(), tmp.getExtension(),
					tmp.getVersion()));
		}
	...
	public Optional<ModelArchiveCoordinate> findBestModelArchive(
			ProjectCoordinate projectCoord, String modelType) {
		// TODO find best model archive needs to be implemented properly
		log.warn("only returning the first match as best model!");
		ModelArchiveCoordinate[] results = findModelArchives(projectCoord,
				modelType);
		return (Optional<ModelArchiveCoordinate>) (results.length == 0 ? absent()
				: of(results[0]));
	}
	...
	public List<Artifact> searchByClassifier(String classifier) {
		...
		try {
			...
			TopDocs matches = searcher.search(new TermQuery(t),
					Integer.MAX_VALUE);
			searcher.close();
	...
	private final class DownloadArtifactTask implements Callable<File> {
		...
		public File call() throws Exception {
			...
			DependencyResult dependencies = system.resolveDependencies(session,
					dependencyRequest);
	...
	private static class TheArtifactOnlyDependencySelector implements
			DependencySelector {
	...
	public void setProxy(String type, String host, int port, String user,
			String pass) {
		Authentication auth = user == null ? null : new Authentication(user,
				pass);
	...
	public Collection<ModelArchiveCoordinate> listModels(String classifier) {
		...
		return Collections2.transform(artifacts,
				new Artifact2ModelArchiveTransformer());
	}

	private static final class Artifact2ModelArchiveTransformer implements
			Function<Artifact, ModelArchiveCoordinate> {
		@Override
		public ModelArchiveCoordinate apply(Artifact a) {

			return new ModelArchiveCoordinate(a.getGroupId(),
					a.getArtifactId(), a.getClassifier(), a.getExtension(),
					a.getVersion());
		}
	}
	 ...
	 * container is used.
	 */
	private class ManualWagonProvider implements
