...
/**
 * The RecommendersModelIndex index is the default implementation for of an {@link IModelArchiveCoordinateResolver}. It
 * internally uses an Apache Lucene index. Clients should rather refer to the interface instead of referencing this
 * class directly.
 */
public class ModelIndex implements IModelArchiveCoordinateResolver, IModelIndex {
    ...
    public Optional<ModelArchiveCoordinate> suggest(ProjectCoordinate coord, String modelType) {
        // TODO find best model archive needs to be implemented properly
        log.warn("only returning the first match as best model!");
        ModelArchiveCoordinate[] results = suggestCandidates(coord, modelType);
        return (Optional<ModelArchiveCoordinate>) (results.length == 0 ? absent() : of(results[0]));
    }

    @Override
    public ModelArchiveCoordinate[] suggestCandidates(ProjectCoordinate coord, String modelType) {
        // TODO it only matches the aid!
        Checks.ensureIsNotNull(modelType);
        // TODO write a new match strategy gid:aid:*
        // Term t0 = new Term(Constants.F_GROUP_ID, groupId);
        Term t1 = new Term(F_SYMBOLIC_NAMES, coord.getArtifactId());
        Term t2 = new Term(F_CLASSIFIER, modelType);
        Set<ModelArchiveCoordinate> results = Sets.newHashSet();
        for (String model : findModelArchiveCoordinatesByTerm(modelType, t1, t2)) {
    ...
    private Set<String> findModelArchiveCoordinatesByTerm(String modelCoordFieldName, Term... terms) {
        if (!isAccessible()) {
    ...
    @Override
    public Collection<ModelArchiveCoordinate> getKnownModels(String modelType) {
    ...
    private Optional<ProjectCoordinate> findProjectCoordinateByTerm(Term... terms) {
        ...
        try {
            ...
            ProjectCoordinate coord = new ProjectCoordinate(tmp.getGroupId(), tmp.getArtifactId(), tmp.getVersion());
            return fromNullable(coord);
        } catch (Exception e) {
            return absent();
        }
    }
