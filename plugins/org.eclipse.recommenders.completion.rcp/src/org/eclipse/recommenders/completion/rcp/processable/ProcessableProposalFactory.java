...
public class ProcessableProposalFactory implements IProcessableProposalFactory {
    ...
    private static Field proposalInfoField = ReflectionUtils.getDeclaredField(AbstractJavaCompletionProposal.class,
            "fProposalInfo").orNull();
    ...
    private static void setProposalInfo(IProcessableProposal res, IJavaCompletionProposal uiProposal) {
        // XXX this method should under no circumstances throw any exception
        if (Checks.isNull(proposalInfoField, res, uiProposal)) {
            return;
        }
        try {
            ProposalInfo info = (ProposalInfo) proposalInfoField.get(uiProposal);
            res.setProposalInfo(info);
        } catch (Exception e) {
            LOG.warn("Failed to set proposal info to '{}'). Returning proposal without additional info.", res);
        }
    }
