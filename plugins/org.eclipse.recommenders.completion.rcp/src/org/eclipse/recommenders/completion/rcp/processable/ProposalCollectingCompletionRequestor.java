...
public class ProposalCollectingCompletionRequestor extends CompletionRequestor {
    ...
    private final Field fProposals = Reflections.getDeclaredField(CompletionProposalCollector.class, "fJavaProposals")
            .orNull();
    ...
    private IJavaCompletionProposal[] createJdtProposals(final CompletionProposal proposal) {
        if (fProposals != null) {
            try {
                ...
                List<IJavaCompletionProposal> list = (List<IJavaCompletionProposal>) fProposals.get(collector);
                int old = list.size();
                collector.accept(proposal);
                int newSize = list.size();
                List<IJavaCompletionProposal> res = list.subList(old, newSize);
