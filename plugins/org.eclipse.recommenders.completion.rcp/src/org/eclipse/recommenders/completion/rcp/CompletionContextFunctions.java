...
public class CompletionContextFunctions {
    ...
    public static Map<CompletionContextKey, ICompletionContextFunction> defaultFunctions() {
        ...
        res.put(JAVA_NON_CONSTRUCTOR_PROPOSALS, new ModifiedJavaProposalsContextFunction(true, false));
        res.put(JAVA_NON_TYPE_PROPOSALS, new ModifiedJavaProposalsContextFunction(false, true));
        res.put(JAVA_NON_CONSTRUCTOR_NON_TYPE_PROPOSALS, new ModifiedJavaProposalsContextFunction(true, true));
    ...
    public static class ModifiedJavaProposalsContextFunction implements ICompletionContextFunction<Object> {

        private final boolean ignoreConstructors;
        private final boolean ignoreTypes;

        public ModifiedJavaProposalsContextFunction(boolean ignoreConstructors, boolean ignoreTypes) {
        ...
        public Object compute(IRecommendersCompletionContext context, CompletionContextKey<Object> key) {
            ...
            ICompilationUnit cu = context.getCompilationUnit();
            ProposalCollectingCompletionRequestor collector = new ProposalCollectingCompletionRequestor(coreContext,
                    ignoreConstructors, ignoreTypes);
            try {
                cu.codeComplete(offset, collector, new TimeDelimitedProgressMonitor(5000));
            } catch (final Exception e) {
                RcpPlugin.logError(e, Messages.LOG_ERROR_EXCEPTION_DURING_CODE_COMPLETION);
