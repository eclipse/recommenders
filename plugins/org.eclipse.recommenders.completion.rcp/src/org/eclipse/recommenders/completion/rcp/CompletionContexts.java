...
public final class CompletionContexts {
    ...
    private static String stripValueOrLinkDelimiters(String string) {
        if (string.startsWith("{@value ") && string.endsWith("}")) {
            int lastIndexOfHash = string.lastIndexOf('#');
            int start = lastIndexOfHash < 0 ? "{@value ".length() : lastIndexOfHash;
            return string.substring(start + 1, string.length() - 1);
        } else if (string.startsWith("{@link ") && string.endsWith("}")) {
            int lastIndexOfHash = string.lastIndexOf('#');
            int start = lastIndexOfHash < 0 ? "{@link ".length() : lastIndexOfHash + 1;
