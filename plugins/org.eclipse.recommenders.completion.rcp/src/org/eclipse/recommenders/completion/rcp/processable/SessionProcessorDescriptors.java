...
public class SessionProcessorDescriptors {
    ...
    public static List<SessionProcessorDescriptor> load(String string, List<SessionProcessorDescriptor> available) {
        ...
        for (String id : StringUtils.split(string, SEPARATOR)) {
            ...
            if (found != null) {
                // SessionProcessorDescriptor descriptor = new AdvisorDescriptor(found);
    ...
    public static boolean isEnabled(SessionProcessorDescriptor d, String enabledProcessors) {
        int index = enabledProcessors.indexOf(d.getId());
        if (index == -1) {
            return false;
        }
        if (index == 0) {
        ...
        if (enabledProcessors.charAt(index - 1) == DISABLED_FLAG) {
            return false;
        }
        return true;
    }
