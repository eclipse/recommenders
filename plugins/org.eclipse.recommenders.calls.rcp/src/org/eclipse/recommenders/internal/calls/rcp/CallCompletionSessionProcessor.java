...
public class CallCompletionSessionProcessor extends SessionProcessor {
    ...
    private boolean findRecommendations() {
        ...
        if (firstMethod != null) {
            overriddenMethod = pcProvider.get().toName(firstMethod).orNull();
        ...
        IMethodName definingMethod = ctx.get(RECEIVER_DEF_BY, null);
        model.setObservedDefiningMethod(definitionKind == DefinitionKind.PARAM ? overriddenMethod : definingMethod);
