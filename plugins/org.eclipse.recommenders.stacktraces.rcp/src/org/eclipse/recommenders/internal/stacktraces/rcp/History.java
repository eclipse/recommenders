...
public class History extends AbstractIdleService {
    ...
    public boolean didSent(ErrorReport report) {
        try {
            ...
            log(HISTORY_NOT_AVAILABLE);
        }
        return false;
    }

    @VisibleForTesting
    protected String identity(ErrorReport report) {
        ...
        return Hashing.murmur3_128().newHasher().putString(json, UTF_8).hash().toString();
    }
    ...
    public void remember(ErrorReport report) {
        ...
        Field fp = new Field(F_IDENTITY, identity, Store.NO, Index.NOT_ANALYZED);
        doc.add(fp);
        try {
