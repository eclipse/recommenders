...
public class ErrorReports {
    ...
    public static void anonymizeStackTrace(ErrorReport report, Settings settings) {
        if (report.getStatus() != null) {
            anonymizeStackTrace(report.getStatus(), settings);
        }
    }

    public static void anonymizeStackTrace(Status status, Settings settings) {
        if (status.getException() != null) {
            anonymizeStackTrace(status.getException(), settings.getWhitelistedPackages());
        }
        for (Status child : status.getChildren()) {
            anonymizeStackTrace(child, settings);
        }
    }

    public static void anonymizeStackTrace(Throwable throwable, List<String> whitelist) {
        if (!isWhitelisted(throwable.getClassName(), whitelist)) {
            throwable.setClassName(HIDDEN);
        }
        for (StackTraceElement e : throwable.getStackTrace()) {
            anonymizeStackTraceElement(e, whitelist);
        }
    }

    public static void anonymizeStackTraceElement(StackTraceElement element, List<String> whitelist) {
