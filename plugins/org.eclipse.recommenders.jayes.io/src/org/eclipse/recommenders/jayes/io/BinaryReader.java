...
public class BinaryReader {
    ...
    private String readString(ByteBuffer buffer) {
        ...
        buffer.get(bytes);
        // BinaryFormatUtils.fastGet(bytes, buffer);
    ...
    private void readBayesNodeDeclaration(BayesNet net, ByteBuffer buffer) {
        ...
        String[] outcomeArray = new String[outcomes];
        for (int i = 0; i < outcomes; i++) {
            outcomeArray[i] = readString(buffer);
        }
        node.addOutcomes(outcomeArray);
     ...
     * @throws IOException
     */
    private void readBayesNodeDefinition(BayesNet net, BayesNode node, ByteBuffer buffer) throws IOException {
        List<BayesNode> parents = readParents(net, buffer);
        node.setParents(parents);
        // BinaryFormatUtils.fastSetParents(node, parents);

        double[] doubles = readProbabilityTable(buffer);
        node.setProbabilities(doubles);
    }

    private double[] readProbabilityTable(ByteBuffer buffer) throws IOException {
        int nrDoubles = buffer.getInt();
        double[] doubles = new double[nrDoubles];
        // getDoubles(buffer, doubles);
        BinaryFormatUtils.fastGet(doubles, buffer);

        return doubles;
    }

    private void getDoubles(ByteBuffer buffer, double[] doubles) {
    ...
    private int[] readParentIds(ByteBuffer buffer) throws IOException {
        ...
        getInts(buffer, ids);
        // BinaryFormatUtils.fastGet(ids, buffer);
        return ids;
    }

    private void getInts(ByteBuffer buffer, int[] ids) {
