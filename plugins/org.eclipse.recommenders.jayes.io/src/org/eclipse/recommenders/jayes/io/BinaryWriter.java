...
/**
 * Writer for a binary representation of a BayesNet. <br/>
 * <br/>
...
public class BinaryWriter implements Closeable {
    ...
    private byte[] writeToArray(BayesNet net) {
        ...
        ByteBuffer buffer = ByteBuffer.allocate(size + HEADER_BYTES);
        putHeader(buffer);
    ...
    /**
     * estimate binary size. Due to UTF-8 using different amounts of bytes per char, this may be over-estimating, but is
