...
public class ProxiesTest {
    ...
    public void testProxyAuthentication() throws URISyntaxException, IOException {
        ...
        when(executor.auth(new AuthScope(new HttpHost(host, PORT), AuthScope.ANY_REALM, "ntlm"),
                new NTCredentials(USER, PASSWORD, Proxies.getWorkstation().orNull(), Proxies.getUserDomain(USER).orNull())))
                        .thenReturn(executor);

        assertThat(Proxies.proxyAuthentication(service, executor, uri), is(equalTo(executor)));
