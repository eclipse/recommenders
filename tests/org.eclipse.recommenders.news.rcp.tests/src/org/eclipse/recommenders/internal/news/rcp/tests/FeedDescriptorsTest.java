...
public class FeedDescriptorsTest {
    ...
    private static FeedDescriptor enabled(String order) {
        IConfigurationElement id = Mockito.mock(IConfigurationElement.class);
        when(id.getAttribute("id")).thenReturn(order);
        return new FeedDescriptor(id, true);
    }

    private static FeedDescriptor disabled(String order) {
        IConfigurationElement id = Mockito.mock(IConfigurationElement.class);
        when(id.getAttribute("id")).thenReturn(order);
        return new FeedDescriptor(id, false);
    }
