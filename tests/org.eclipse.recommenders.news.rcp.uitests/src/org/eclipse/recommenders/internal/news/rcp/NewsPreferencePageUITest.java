...
public class NewsPreferencePageUITest {
    private static final int PREFERENCE_PAGE_ID = -2;
    ...
    private static final String INVALID_FEED_URL = "http://eclipse"; //$NON-NLS-1$
    private static final String CHARACTERS_AND_DIGITS = "ab35g"; //$NON-NLS-1$
    ...
    public void testAddCustomFeedWithDuplicateURL() {
        ...
        assertThat(bot.text(" " + MessageFormat.format(Messages.FEED_DIALOG_ERROR_DUPLICATE_FEED,
                "org.eclipse.recommenders.news.rcp.planet.eclipse")), is(notNullValue()));
    }
    ...
    private void openNewsPreferencePage() {
        ...
        if (Platform.getOS().equals(Platform.OS_MACOSX)) {
                ...
                public void run() {
                    ...
                    if (window != null) {
                        ...
                        for (MenuItem item : appMenu.getItems()) {
                            if (item.getID() == PREFERENCE_PAGE_ID) {
            ...
            clickMainMenu("Window", "Preferences"); //$NON-NLS-1$ //$NON-NLS-2$
        }
    ...
    private static SWTBotShell getMainShell() {
        for (SWTBotShell shellBot : bot.shells()) {
            if (shellBot.getText().toLowerCase().contains("eclipse")
                    || shellBot.getText().toLowerCase().contains("resource")) {
                return shellBot;
            }
        }
