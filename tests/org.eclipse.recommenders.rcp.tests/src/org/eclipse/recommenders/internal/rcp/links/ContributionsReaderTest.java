...
public class ContributionsReaderTest {
    ...
    public void testContributionLinkOptionalAttributes() {
        IConfigurationElement configElement = mockConfigElement(CONTRIBUTION_ELEMENT, ImmutableMap.of(
                ...
                COMMAND_ID_ATTRIBUTE, "org.example.command.id"));
        ...
        assertThat(link.getCommandId(), is("org.example.command.id"));
    ...
    public void testContributionLinkWithInvalidPriority() {
        IConfigurationElement firstConfigElement = mockConfigElement(CONTRIBUTION_ELEMENT, ImmutableMap.of(
                PREF_PAGE_ID_ATTRIBUTE, "org.example.pref.page.id", LABEL_ATTRIBUTE, "<a>some Label</a>",
                COMMAND_ID_ATTRIBUTE, "org.exmaple.first.command.id", PRIORITY_ATTRIBUTE, "invalid", ICON_ELEMENT,
                DEFAULT_LINK_ICON));

        IConfigurationElement secondConfigElement = mockConfigElement(CONTRIBUTION_ELEMENT, ImmutableMap.of(
                PREF_PAGE_ID_ATTRIBUTE, "org.example.pref.page.id", LABEL_ATTRIBUTE, "<a>other Label</a>",
        ...
        assertThat(link.getCommandId(), is("org.example.second.command.id"));
        assertThat(link.getPriority(), is(10));
        assertThat(link.getIcon(), is(notNullValue()));
    }

    @Test
    public void testContributionReaderConsistent() {
        IConfigurationElement firstConfigElement = mockConfigElement(CONTRIBUTION_ELEMENT, ImmutableMap.of(
    ...
    public void testMultipleLinksContribution() {
        ...
        IConfigurationElement secondConfigElement = mockConfigElement(CONTRIBUTION_ELEMENT, ImmutableMap.of(
                PREF_PAGE_ID_ATTRIBUTE, "org.example.pref.page.id", LABEL_ATTRIBUTE, "<a>second Label</a>",
                COMMAND_ID_ATTRIBUTE, "org.example.second.command.id", PRIORITY_ATTRIBUTE, "0", ICON_ELEMENT,
                DEFAULT_LINK_ICON));
    ...
    public void testLinkContributionForDifferentPreferencePages() {
        ...
        assertThat(firstPageLink.getCommandId(), is("org.example.first.command.id"));
