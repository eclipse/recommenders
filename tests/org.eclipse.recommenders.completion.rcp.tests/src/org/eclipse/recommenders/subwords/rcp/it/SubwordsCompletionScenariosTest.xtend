...
class SubwordsCompletionProposalComputerIntegrationTest {
    ...
    /**
     * @see <a href="https://bugs.eclipse.org/bugs/show_bug.cgi?id=435745">Bug 435745</a>
     */
    @Test
    def void test016_ConstructorExpression() {
        val code = method('''new Ot$''')
        exerciseAndVerifyLenient(code, asList("Object("))
    }
     ...
     */
    @Test
    def void test017_ReturnTypeOfMethodDeclaration() {
        val code = classbody('''public Ot$ method() {}''')
        exerciseAndVerifyLenient(code, asList("Object"))
    }
     ...
     */
    @Test
    def void test018_ParameterTypeOfMethodDeclaration() {
        val code = classbody('''public void method(Ot$) {}''')
        exerciseAndVerifyLenient(code, asList("Object"))
    }
    ...
    def smokeTest(CharSequence code) {
        ...
        for (completionIndex : struct.second) {
            ...
            val prefs = Mockito.mock(CompletionRcpPreferences)
            val processor = new SubwordsSessionProcessor(new CachingAstProvider(), new SubwordsRcpPreferences())
            Mockito.when(prefs.getSessionProcessors).thenReturn(
                Collections.singletonList(
                    new SessionProcessorDescriptor("subwords", "name", "desc", null, 0, true, "", processor)));
            val sut = new MockedIntelligentCompletionProposalComputer(processor, prefs)
            sut.sessionStarted
    ...
    def exerciseAndVerify(CharSequence code, List<String> expected) {
        ...
        for (e : expected) {
            val match = actual.findFirst(
                p|CompletionContexts.getPrefixMatchingArea(p.displayString).startsWith(e))
            if (null == match) {
                println();
            }
    ...
    def getTokensBetweenLastWhitespaceAndFirstOpeningBracket(String string) {
    ...
    def List<IJavaCompletionProposal> exercise(CharSequence code) {
        ...
        Mockito.when(prefs.getSessionProcessors).thenReturn(
            Collections.singletonList(
                new SessionProcessorDescriptor("base", "name", "desc", null, 0, true, "", processor)));
        val sut = new MockedIntelligentCompletionProposalComputer(processor, prefs)
        sut.sessionStarted
        stopwatch.start
