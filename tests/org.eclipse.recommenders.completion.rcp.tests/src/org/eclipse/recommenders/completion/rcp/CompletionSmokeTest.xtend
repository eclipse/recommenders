...
import org.eclipse.recommenders.calls.rcp.CallCompletionSessionProcessor
import org.eclipse.recommenders.completion.rcp.processable.ProcessableProposalFactory
import org.eclipse.recommenders.completion.rcp.processable.SessionProcessor
import org.eclipse.recommenders.completion.rcp.processable.SessionProcessorDescriptor
import org.eclipse.recommenders.internal.completion.rcp.IntelligentCompletionProposalComputer
...
import org.eclipse.recommenders.overrides.rcp.OverrideCompletionSessionProcessor
...
class CompletionSmokeTest {
    ...
    def void exercise(CharSequence scenario) {
        ...
        val pcp = mock(ProjectCoordinateProvider)
    ...
    def createSut(ProjectCoordinateProvider pcp, JavaElementResolver jer) {
        switch processor {
            case "calls": {
                val mp = mock(ICallModelProvider)
                when(mp.acquireModel(anyObject())).thenReturn(Optional.<ICallModel>of(NullCallModel.NULL_MODEL))
                return new CallCompletionSessionProcessor(pcp, mp, jer)
            }
            case "overrides": {
    ...
    static def newCallComputer() {
        ...
        val mp = mock(ICallModelProvider)
        when(mp.acquireModel(anyObject())).thenReturn(Optional.<ICallModel>of(NullCallModel.NULL_MODEL))
        val sut = new CallCompletionSessionProcessor(pcp, mp, jer)
