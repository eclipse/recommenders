...
public class AstNodeUtilsTest {
    ...
    public void testSameSimpleNameOnSimpleTypes() {
        ...
        // test simple type with simple name
        assertTrue(sameSimpleName(newSimpleType(newSimpleName()), crParam));
        // test simple type with qualified name
        assertTrue(sameSimpleName(newSimpleType(newQualifiedName()), crParam));
        // test qualified type
        assertTrue(sameSimpleName(newQualifiedType(), crParam));
        // test parameterized type
        assertTrue(sameSimpleName(newParameterizedType(), crParam));
    }
    ...
    private QualifiedType newQualifiedType() {
        QualifiedType type = ast.newQualifiedType(ast.newSimpleType(newSimpleName()), newSimpleName());
        return ast.newQualifiedType(type, newSimpleName());
    }

    private SimpleName newSimpleName() {
        return ast.newSimpleName("Object");
    }
