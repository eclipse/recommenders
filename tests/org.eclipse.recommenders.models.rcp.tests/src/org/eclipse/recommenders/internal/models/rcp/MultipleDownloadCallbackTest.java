...
public class MultipleDownloadCallbackTest {
    ...
    private static final Double[] REMAINDER_SPLIT = new Double[] { 1.0 };
    ...
    public void testAllWorkDoneIfFinished() {
        ...
        assertCapturedSequence(workedUnits, skippedDownload(50.0), skippedDownload(50.0));
    }
    ...
    public void testAllWorkDoneIfFinishedWithOneDownload() {
        ...
        verify(monitor, atLeastOnce()).internalWorked(workedUnits.capture());
        assertCapturedSequence(workedUnits, download(50.0), skippedDownload(50.0));
    }
    ...
    private static void assertCapturedSequence(ArgumentCaptor<Double> captor, Double[]... expectedValues) {
        Double[] allExpectedValues = concatenate(expectedValues);
        List<Double> allValues = captor.getAllValues();
        assertThat(allValues, Matchers.equalTo(Arrays.asList(allExpectedValues)));
    }

    private static Double[] concatenate(Double[][] expectedValues) {
        int size = 0;
        for (int i = 0; i < expectedValues.length; i++) {
            size += expectedValues[i].length;
        }
        Double[] allExpectedValues = new Double[size];
        int pos = 0;
        for (int i = 0; i < expectedValues.length; i++) {
            for (int j = 0; j < expectedValues[i].length; j++) {
                allExpectedValues[pos++] = expectedValues[i][j];
            }
        }
        return allExpectedValues;
    }

    private static Double[] download(Double... expectedValues) {
        return expectedValues;
    }

    private static Double[] progress(Double... expectedValues) {
        return expectedValues;
    }

    private static Double[] skippedDownload(Double... expectedValues) {
        return expectedValues;
    }
