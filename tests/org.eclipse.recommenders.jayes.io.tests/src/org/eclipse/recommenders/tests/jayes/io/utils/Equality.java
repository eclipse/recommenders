...
public class Equality {
    ...
    public static Matcher<BayesNet> equals(final BayesNet net1) {
        return new TypeSafeDiagnosingMatcher<BayesNet>() {
            ...
            protected boolean matchesSafely(BayesNet net2, Description mismatchDescription) {
                ...
                for (int i = 0; i < net1.getNodes().size(); i++) {
                    if (!Equality.equals(net1.getNode(i)).matches(net2.getNode(i))) {
                        Equality.equals(net1.getNode(i)).describeMismatch(net2.getNode(i),
                                mismatchDescription);
                        return false;
                    }
                }
    ...
    public static Matcher<BayesNode> equals(final BayesNode node1) {
        return new TypeSafeDiagnosingMatcher<BayesNode>() {
            ...
            protected boolean matchesSafely(BayesNode node2, Description mismatchDescription) {
                ...
                if (!IsCloseTo.isCloseTo(node1.getProbabilities(), Math.pow(10, -10)).matches(node2.getProbabilities())) {
