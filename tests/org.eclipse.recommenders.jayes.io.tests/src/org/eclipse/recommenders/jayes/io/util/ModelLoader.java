...
public class ModelLoader {
    ...
    private static final Logger LOGGER = LoggerFactory
            .getLogger(ModelLoader.class);

    public ModelLoader(String modelArtifact) throws ClassNotFoundException,
            IOException {
        networks = loadModels(modelArtifact);
        LOGGER.info("successfully loaded " + networks.size() + " models");
    }

    private List<BayesNet> loadModels(String modelArtifact)
            throws ClassNotFoundException, IOException {
        ModelRepository repo = new ModelRepository(
                new File("target/local-repo"), REMOTE_MODEL_REPO);
        Optional<File> models = repo.resolve(
                ModelCoordinate.valueOf("jre:jre:call:zip:1.0.0"), true);
        return readModelFromZip(models.get());
    }

    private List<BayesNet> readModelFromZip(File zip) throws IOException,
