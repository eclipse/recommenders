...
public class TypesCompletionSessionProcessorTest {
    ...
    private static final String LINKED_LIST_WITH_GENERIC_SIGNATURE = "Ljava.util.LinkedList<LJava.lang.String>;";
    ...
    @Test
    public void testProposalWithGenerics() throws Exception {
        IRecommendersCompletionContext context = setUpCompletionScenario("L", LIST);
        ...
        ProposalProcessorManager manager = mock(ProposalProcessorManager.class);
        IProcessableProposal OptionalProcessableProposal = mockIProcessableProposal(manager,
                CompletionProposal.TYPE_REF, LINKED_LIST_WITH_GENERIC_SIGNATURE);

        sut.process(OptionalProcessableProposal);

        verify(OptionalProcessableProposal).setTag(ProposalTag.RECOMMENDERS_SCORE, BOOST);
        verify(manager, times(1)).addProcessor(processorWithBoost(BOOST));
        verify(manager, times(1)).addProcessor(isA(OverlayImageProposalProcessor.class));
        verifyNoMoreInteractions(manager);
    }
