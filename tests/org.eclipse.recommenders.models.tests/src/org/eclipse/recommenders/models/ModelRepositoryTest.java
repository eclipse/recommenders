...
public class ModelRepositoryTest {
    ...
    public void testSuccessfulDownload() throws Exception {
        ...
        verify(transporter, times(1)).get((GetTask) argThat(hasProperty("location", equalTo(METADATA_XML))));
        verify(transporter, times(1)).get((GetTask) argThat(hasProperty("location", equalTo(METADATA_XML_SHA1))));
        verify(transporter, times(1)).get((GetTask) argThat(hasProperty("location", equalTo(EXAMPLE_MODEL_ZIP))));
        verify(transporter, times(1)).get((GetTask) argThat(hasProperty("location", equalTo(EXAMPLE_MODEL_ZIP_SHA1))));
    }
    ...
    public void testChecksumMismatchForMetadata() throws Exception {
        ...
        verify(transporter, atMost(2)).get((GetTask) argThat(hasProperty("location", equalTo(METADATA_XML))));
        verify(transporter, atMost(2)).get((GetTask) argThat(hasProperty("location", equalTo(METADATA_XML_SHA1))));
        verify(transporter, never()).get((GetTask) argThat(hasProperty("location", equalTo(EXAMPLE_MODEL_ZIP))));
        verify(transporter, never()).get((GetTask) argThat(hasProperty("location", equalTo(EXAMPLE_MODEL_ZIP_SHA1))));
    }
    ...
    public void testChecksumMissingForMetadata() throws Exception {
        ...
        Transporter transporter = mockTransporter(ImmutableMap.of(
                METADATA_XML, new SucessfulDownload(metadata),
                METADATA_XML_SHA1, new MissingResource(),
                EXAMPLE_MODEL_ZIP, new SucessfulDownload(model),
                EXAMPLE_MODEL_ZIP_SHA1, new SucessfulDownload(sha1(model))));
        /* @formatter:on */
        ...
        assertThat(resolvedModel.isPresent(), is(false));
        // FIXME Aether re-tries downloading the missing maven-metadata.xml
        verify(transporter, times(1)).get((GetTask) argThat(hasProperty("location", equalTo(METADATA_XML))));
        verify(transporter, times(1)).get((GetTask) argThat(hasProperty("location", equalTo(METADATA_XML_SHA1))));
        verify(transporter, never()).get((GetTask) argThat(hasProperty("location", equalTo(EXAMPLE_MODEL_ZIP))));
        verify(transporter, never()).get((GetTask) argThat(hasProperty("location", equalTo(EXAMPLE_MODEL_ZIP_SHA1))));
    ...
    public void testChecksumMismatchForModel() throws Exception {
        ...
        verify(transporter, times(1)).get((GetTask) argThat(hasProperty("location", equalTo(METADATA_XML))));
        verify(transporter, times(1)).get((GetTask) argThat(hasProperty("location", equalTo(METADATA_XML_SHA1))));
        verify(transporter, atMost(2)).get((GetTask) argThat(hasProperty("location", equalTo(EXAMPLE_MODEL_ZIP))));
        verify(transporter, atMost(2)).get((GetTask) argThat(hasProperty("location", equalTo(EXAMPLE_MODEL_ZIP_SHA1))));
    }
    ...
    public void testChecksumMissingForModel() throws Exception {
        ...
        Transporter transporter = mockTransporter(ImmutableMap.of(
                ...
                METADATA_XML_SHA1, new SucessfulDownload(sha1(metadata)),
                EXAMPLE_MODEL_ZIP, new SucessfulDownload(model),
                EXAMPLE_MODEL_ZIP_SHA1, new MissingResource()));
        /* @formatter:on */
