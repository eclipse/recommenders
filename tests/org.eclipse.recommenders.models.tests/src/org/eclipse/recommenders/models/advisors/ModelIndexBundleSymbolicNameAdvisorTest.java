...
public class ModelIndexBundleSymbolicNameAdvisorTest {
    ...
    public void testValidJAR() throws IOException {
        ...
        when(mockedOsgiAdvisor.doSuggest(dependencyInfo)).thenReturn(Optional.fromNullable(COORDINATE));
        ...
        assertEquals(COORDINATE, optionalProjectCoordinate.get());
    }
    ...
    public void testInvalidJAR() throws IOException {
        ...
        OsgiManifestAdvisor mockedOsgiAdvisor = spy(new OsgiManifestAdvisor());
        when(mockedOsgiAdvisor.doSuggest(dependencyInfo)).thenReturn(fromNullable(COORDINATE));
